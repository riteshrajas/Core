...
> {
  ...
  // the turn returns — otherwise `APEX list` shows stale state for the gap.
  ...
  if (
    ...
    const p = jobClassifierModule!
      .classifyAndWriteState(process.env.APEX_JOB_DIR, turnAssistantMessages)
      .catch(err => {
  ...
  if (!isBareMode()) {
    ...
    if (!isEnvDefinedFalsy(process.env.APEX_CODE_ENABLE_PROMPT_SUGGESTION)) {
