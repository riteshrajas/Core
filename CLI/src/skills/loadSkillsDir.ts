...
/**
 * Returns a APEX config directory path for a given source.
 */
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.APEX', dir)
    case 'userSettings':
      return join(getAPEXConfigHomeDir(), dir)
...
/**
 * Parse paths frontmatter from a skill, using the same format as APEX.md rules.
...
export function createSkillCommand({
  ...
  return {
    ...
    async getPromptForCommand(args, toolUseContext) {
      ...
      // Replace ${APEX_SKILL_DIR} with the skill's own directory so bash
      ...
      if (baseDir) {
        ...
        finalContent = finalContent.replace(/\$\{APEX_SKILL_DIR\}/g, skillDir)
      }

      // Replace ${APEX_SESSION_ID} with the current session ID
      ...
      // ${APEX_SKILL_DIR} is meaningless for MCP skills anyway.
      if (loadedFrom !== 'mcp') {
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    const userSkillsDir = join(getAPEXConfigHomeDir(), 'skills')
    const managedSkillsDir = join(getManagedFilePath(), '.APEX', 'skills')
    ...
    const [
      ...
      isEnvTruthy(process.env.APEX_CODE_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.APEX', 'skills')
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored — blocks e.g. node_modules/pkg/.APEX/skills from
