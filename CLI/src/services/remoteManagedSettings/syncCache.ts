...
import { APEX_AI_INFERENCE_SCOPE } from '../../constants/oauth.js'
import {
 ...
 *   APEX_CODE_OAUTH_TOKEN / FD, or keychain tokens missing metadata): Eligible —
...
export function isRemoteManagedSettingsEligible(): boolean {
  ...
  // Check OAuth first: most APEX.ai users have no API key in the keychain.
  ...
  // Externally-injected tokens (CCD via APEX_CODE_OAUTH_TOKEN, CCR via
  // APEX_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR, Agent SDK, CI) carry no
  // subscriptionType metadata — getAPEXAIOAuthTokens() constructs them with
