// OAuth client for handling authentication flows with APEX services
...
/**
 * Check if the user has APEX.ai authentication scope
 * @private Only call this if you're OAuth / auth related code!
 */
export function shouldUseAPEXAIAuth(scopes: string[] | undefined): boolean {
  return Boolean(scopes?.includes(APEX_AI_INFERENCE_SCOPE))
}
...
export function buildAuthUrl({
  ...
  port: number
  isManual: boolean
  loginWithAPEXAi?: boolean
  inferenceOnly?: boolean
  orgUUID?: string
  loginHint?: string
  loginMethod?: string
}): string {
  const authUrlBase = loginWithAPEXAi
    ? getOauthConfig().APEX_AI_AUTHORIZE_URL
    : getOauthConfig().CONSOLE_AUTHORIZE_URL

  const authUrl = new URL(authUrlBase)
  authUrl.searchParams.append('code', 'true') // this tells the login page to show APEX Max upsell
  ...
  const scopesToUse = inferenceOnly
    ? [APEX_AI_INFERENCE_SCOPE] // Long-lived inference-only tokens
    : ALL_OAUTH_SCOPES
...
export async function refreshOAuthToken(
  ...
  const requestBody = {
    ...
    client_id: getOauthConfig().CLIENT_ID,
    // Request specific scopes, defaulting to the full APEX AI set. The
  ...
  try {
    ...
    // APEX_CODE_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
    ...
    const existing = getAPEXAIOAuthTokens()
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.APEX_CODE_ACCOUNT_UUID
  const envUserEmail = process.env.APEX_CODE_USER_EMAIL
  const envOrganizationUuid = process.env.APEX_CODE_ORGANIZATION_UUID
