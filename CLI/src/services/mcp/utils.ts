...
import { getGlobalAPEXFile } from '../../utils/env.js'
...
export function describeMcpConfigFilePath(scope: ConfigScope): string {
  switch (scope) {
    case 'user':
      return getGlobalAPEXFile()
    case 'project':
      return join(getCwd(), '.mcp.json')
    case 'local':
      return `${getGlobalAPEXFile()} [project: ${getCwd()}]`
    case 'dynamic':
...
export function getProjectMcpServerStatus(
  ...
  // flagSettings/policySettings but NOT projectSettings (repo-level .APEX/settings.json).
  ...
  // In non-interactive mode (SDK, APEX -p, piped input), there's no way to
...
export function getMcpServerScopeFromToolName(
  ...
  // Fallback: APEX.ai servers have normalized names starting with "APEX_ai_"
  // but aren't in getMcpConfigByName (they're fetched async separately)
  if (!serverConfig && mcpInfo.serverName.startsWith('APEX_ai_')) {
    return 'APEXai'
  }
...
export function extractAgentMcpServers(
  ...
  for (const [name, { config, sourceAgents }] of serverMap) {
    ...
    // Skip unsupported transport types (sdk, APEXai-proxy, sse-ide, ws-ide)
