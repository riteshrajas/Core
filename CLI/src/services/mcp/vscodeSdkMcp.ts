 ...
 * notify VSCode when files are edited or written by APEX.
 */
...
export function setupVscodeSdkMcp(sdkClients: MCPServerConnection[]): void {
  const client = sdkClients.find(client => client.name === 'APEX-vscode')
  ...
  if (client && client.type === 'connected') {
    ...
    const gates: Record<string, boolean | string> = {
      ...
      // In-band OAuth via APEX_authenticate (vs. extension-native PKCE).
