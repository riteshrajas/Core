 ...
 *   - sends `notifications/APEX/channel` notifications for inbound — this file
 *
 ...
 * Requires APEX.ai OAuth auth — API key users are blocked until
...
export const ChannelMessageNotificationSchema = lazySchema(() =>
  z.object({
    method: z.literal('notifications/APEX/channel'),
    params: z.object({
      content: z.string(),
 ...
 * this declare `capabilities.experimental['APEX/channel/permission']` and
 * emit this event INSTEAD of relaying "yes tbxkq" as text via
 * notifications/APEX/channel. Explicit opt-in per server — a channel that
...
export const CHANNEL_PERMISSION_METHOD =
  'notifications/APEX/channel/permission'
...
export const CHANNEL_PERMISSION_REQUEST_METHOD =
  'notifications/APEX/channel/permission_request'
...
export function gateChannelServer(
  ...
  // Channel servers declare `experimental['APEX/channel']: {}` (MCP's
  // presence-signal idiom — same as `tools: {}`). Truthy covers `{}` and
  // `true`; absent/undefined/explicit-`false` all fail. Key matches the
  // notification method namespace (notifications/APEX/channel).
  if (!capabilities?.experimental?.['APEX/channel']) {
    return {
      action: 'skip',
      kind: 'capability',
      reason: 'server did not declare APEX/channel capability',
    }
  }
  ...
  if (!getAPEXAIOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires APEX.ai authentication (run /login)',
    }
  }
