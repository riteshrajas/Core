...
import { fetchAPEXAIMcpConfigsIfEligible } from './APEXai.js'
...
/**
 * CCR proxy URL path markers. In remote sessions, APEX.ai connectors arrive
 ...
 * plugin deduplication. Ignores env (plugins always inject APEX_PLUGIN_ROOT)
...
/**
 * Filter APEX.ai connectors, dropping any whose signature matches an enabled
 * manually-configured server. Manual wins: a user who wrote .mcp.json or ran
 * `APEX mcp add` expressed higher intent than a connector toggled in the web UI.
 *
 * Connector keys are `APEX.ai <DisplayName>` so they never key-collide with
 ...
 * `mcp__APEX_ai_Slack__*` both hitting mcp.slack.com, ~600 chars/turn wasted).
 *
 ...
 */
export function dedupAPEXAiMcpServers(
  APEXAiServers: Record<string, ScopedMcpServerConfig>,
  manualServers: Record<string, ScopedMcpServerConfig>,
): {
  servers: Record<string, ScopedMcpServerConfig>
  suppressed: Array<{ name: string; duplicateOf: string }>
  ...
  const servers: Record<string, ScopedMcpServerConfig> = {}
  const suppressed: Array<{ name: string; duplicateOf: string }> = []
  for (const [name, config] of Object.entries(APEXAiServers)) {
    const sig = getMcpServerSignature(config)
    const manualDup = sig !== null ? manualSigs.get(sig) : undefined
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing APEX.ai connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
 ...
 * in getAPEXCodeMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export async function addMcpConfig(
  ...
  // Block reserved server name "APEX-in-chrome"
  if (isAPEXInChromeMCPServer(name)) {
  ...
  switch (scope) {
    ...
    case 'APEXai':
      throw new Error('Cannot add MCP server to scope: APEXai')
  }
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getAPEXCodeMcpConfigs().
...
/**
 * Get APEX Code MCP configurations (excludes APEX.ai servers from the
 ...
 * APEX.ai connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns APEX Code server configurations with appropriate scopes
 */
export async function getAPEXCodeMcpConfigs(
...
/**
 * Get all MCP configurations across all scopes, including APEX.ai servers.
 * This may be slow due to network calls - use getAPEXCodeMcpConfigs() for fast startup.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load APEX.ai servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getAPEXCodeMcpConfigs()
  }

  // Kick off the APEX.ai fetch before getAPEXCodeMcpConfigs so it overlaps
  // with loadAllPluginsCacheOnly() inside. Memoized — the awaited call below is a cache hit.
  const APEXaiPromise = fetchAPEXAIMcpConfigsIfEligible()
  const { servers: APEXCodeServers, errors } = await getAPEXCodeMcpConfigs(
    {},
    APEXaiPromise,
  )
  const { allowed: APEXaiMcpServers } = filterMcpServersByPolicy(
    await APEXaiPromise,
  )

  // Suppress APEX.ai connectors that duplicate an enabled manual server.
  // Keys never collide (`slack` vs `APEX.ai Slack`) so the merge below
  // won't catch this — need content-based dedup by URL signature.
  const { servers: dedupedAPEXAi } = dedupAPEXAiMcpServers(
    APEXaiMcpServers,
    APEXCodeServers,
  )

  // Merge with APEX.ai having lowest precedence
  const servers = Object.assign({}, dedupedAPEXAi, APEXCodeServers)
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://code.APEX.com/docs/en/mcp#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
...
export function areMcpConfigsAllowedWithEnterpriseMcpConfig(
  ...
  // what the best way to do this is. In the meantime, we are limiting this to APEX-vscode for now to
  ...
  return Object.values(configs).every(
    c => c.type === 'sdk' && c.name === 'APEX-vscode',
  )
}
