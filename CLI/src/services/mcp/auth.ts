 ...
 * Used by APEXAuthProvider for metadata discovery and token refresh.
 ...
 *
 * Per RFC 7009, public clients (like APEX Code) should authenticate by including
 ...
 * `APEX mcp xaa setup`). Per-server config is just `oauth.xaa: true`
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'APEX mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
  ...
  try {
    ...
    // (instead of APEXAuthProvider.saveTokens) to avoid instantiating the
...
export async function performMCPOAuthFlow(
  ...
  // rest of CC's transport wiring (APEXAuthProvider.tokens() in client.ts)
  // works unchanged.
  //
  ...
  //
  // Servers with `oauth.xaa` but APEX_CODE_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set APEX_CODE_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
  ...
  try {
    ...
    const provider = new APEXAuthProvider(
      serverName,
      serverConfig,
      redirectUri,
      true,
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to APEX Code.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
 ...
 * to the PKCE flow. See github.com/anthropics/APEX-code/issues/28258.
 */
export function wrapFetchWithStepUpDetection(
  baseFetch: FetchLike,
  provider: APEXAuthProvider,
): FetchLike {
...
export class APEXAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `APEX Code (${this.serverName})`,
   ...
   * but this is the shape APEX.md flags under "Token/auth caching across
  ...
  async refreshAuthorization(
    ...
    const lockfilePath = join(APEXDir, `mcp-refresh-${sanitizedKey}.lock`)
