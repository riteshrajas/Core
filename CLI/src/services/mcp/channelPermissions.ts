 ...
 * reply and emits notifications/APEX/channel/permission with
 ...
 * capabilities.experimental['APEX/channel/permission'].
 *
 * Kenneth's "would this let APEX self-approve?": the approving party is
 * the human via the channel, not APEX. But the trust boundary isn't the
...
export type ChannelPermissionCallbacks = {
   ...
   *  (notifications/APEX/channel/permission). Returns true if the ID
 ...
 * reply and emits notifications/APEX/channel/permission with {request_id,
...
>(
  ...
  return clients.filter(
    (c): c is T & { type: 'connected' } =>
      ...
      c.capabilities?.experimental?.['APEX/channel'] !== undefined &&
      c.capabilities?.experimental?.['APEX/channel/permission'] !== undefined,
  )
}

/**
 * Factory for the callbacks object. The pending Map is closed over — NOT
 * module-level (per src/APEX.md), NOT in AppState (functions-in-state
 ...
 * (notifications/APEX/channel/permission) with the structured payload.
