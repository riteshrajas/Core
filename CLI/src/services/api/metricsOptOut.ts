...
import { hasProfileScope, isAPEXAISubscriber } from '../../utils/auth.js'
...
// N `APEX -p` invocations into ~1 API call/day.
...
async function _fetchMetricsEnabled(): Promise<MetricsEnabledResponse> {
  ...
  const endpoint = `https://api.anthropic.com/api/APEX_code/organizations/metrics_enabled`
...
export async function checkMetricsEnabled(): Promise<MetricsStatus> {
  ...
  if (isAPEXAISubscriber() && !hasProfileScope()) {
    return { enabled: false, hasError: false }
  }
