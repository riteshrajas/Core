...
import { getMaxOutputTokensForModel } from '../api/APEX.js'
...
export function getEffectiveContextWindowSize(model: string): number {
  ...
  const autoCompactWindow = process.env.APEX_CODE_AUTO_COMPACT_WINDOW
...
export function getAutoCompactThreshold(model: string): number {
  ...
  const envPercent = process.env.APEX_AUTOCOMPACT_PCT_OVERRIDE
  if (envPercent) {
...
export function calculateTokenWarningState(
  ...
  const blockingLimitOverride = process.env.APEX_CODE_BLOCKING_LIMIT_OVERRIDE
...
export async function shouldAutoCompact(
  ...
  // APEX_CONTEXT_COLLAPSE env override is honored here too. require()
