...
// Skills can be large (verify=18.7KB, APEX-api=20.1KB). Previously re-injected
  ...
  try {
    if (promptCacheSharingEnabled) {
      try {
        ...
        // budget_tokens via Math.min(budget, maxOutputTokens-1) in APEX.ts,
...
function shouldExcludeFromPostCompactRestore(
  ...
  // Exclude all types of APEX.md files
  // TODO: Refactor to use isMemoryFilePath() from APEXmd.ts for consistency
  // and to also match child directory memory files (.APEX/rules/*.md, etc.)
  try {
