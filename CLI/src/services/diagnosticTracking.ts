...
class DiagnosticsTrackingError extends APEXError {}
...
export class DiagnosticTrackingService {
  ...
  // Map<normalizedPath, lastAPEXFsRightDiagnostics>
  ...
  /**
   * Get new diagnostics from file://, _APEX_fs_right, and _APEX_fs_ URIs that aren't in the baseline.
  ...
  async getNewDiagnostics(): Promise<DiagnosticFile[]> {
    ...
    const diagnosticsForAPEXFsRightUrisWithBaselinesMap = new Map<
      string,
      DiagnosticFile
    >()
    allDiagnosticFiles
      .filter(file => this.baseline.has(this.normalizeFileUri(file.uri)))
      .filter(file => file.uri.startsWith('_APEX_fs_right:'))
      .forEach(file => {
        diagnosticsForAPEXFsRightUrisWithBaselinesMap.set(
    ...
    for (const file of diagnosticsForFileUrisWithBaselines) {
      ...
      // Get the _APEX_fs_right file if it exists
      const APEXFsRightFile =
        diagnosticsForAPEXFsRightUrisWithBaselinesMap.get(normalizedPath)
      ...
      if (APEXFsRightFile) {
        ...
        if (
          ...
          !this.areDiagnosticArraysEqual(
            previousRightDiagnostics,
            APEXFsRightFile.diagnostics,
          )
        ) {
          fileToUse = APEXFsRightFile
        }
