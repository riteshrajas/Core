...
import { getAPIMetadata } from './api/APEX.js'
...
export let currentLimits: APEXAILimits = {
  status: 'allowed',
...
type StatusChangeListener = (limits: APEXAILimits) => void
export const statusListeners: Set<StatusChangeListener> = new Set()

export function emitStatusChange(limits: APEXAILimits) {
  ...
  logEvent('tengu_APEXai_limits_status_changed', {
    status:
...
export async function checkQuotaStatus(): Promise<void> {
  ...
  if (!shouldProcessRateLimits(isAPEXAISubscriber())) {
    return
  }
  ...
  // headers (APEX.ts), so skip this pre-check API call.
 ...
 * Returns APEXAILimits if a threshold was surpassed, null otherwise.
 */
 ...
 * Returns APEXAILimits if thresholds are exceeded, null otherwise.
 */
...
export function extractQuotaStatusFromHeaders(
  ...
  const isSubscriber = isAPEXAISubscriber()
  ...
  if (!shouldProcessRateLimits(isSubscriber)) {
    ...
    if (currentLimits.status !== 'allowed' || currentLimits.resetsAt) {
      const defaultLimits: APEXAILimits = {
        status: 'allowed',
        unifiedRateLimitFallbackAvailable: false,
        isUsingOverage: false,
      }
