...
// OAuth credentials as APEX Code.  The endpoint uses conversation_engine
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as APEX Code — available when the
  // user is authenticated with Anthropic (APEX.ai subscriber or has
  // valid OAuth tokens).
...
export async function connectVoiceStream(
  ...
  // visibility.external: true). We target that host instead of APEX.ai
  // because the APEX.ai CF zone uses TLS fingerprinting and challenges
  // non-browser clients (anthropics/APEX-code#34094). Same private-api
  // pod, same OAuth Bearer auth — just a CF zone that doesn't block us.
  // Desktop dictation still uses APEX.ai (Swift URLSession has a
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (anthropics/APEX-code#40510).
