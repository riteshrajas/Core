/**
 * Main entrypoint for APEX Code Agent SDK types.
 *
 ...
 *
 * If your SDK MCP calls will run longer than 60s, override APEX_CODE_STREAM_CLOSE_TIMEOUT
 */
...
/**
 * A scheduled task from `<dir>/.APEX/scheduled_tasks.json`.
 * @internal
 */
...
/**
 * Watch `<dir>/.APEX/scheduled_tasks.json` and yield events as tasks fire.
 *
...
/**
 * A user message typed on APEX.ai, extracted from the bridge WS.
 * @internal
 */
...
/**
 * Hold a APEX.ai remote-control bridge connection from a daemon process.
 *
 ...
 * APEX.ai keeps the same session. Contrast with `query.enableRemoteControl`
 * which puts the WS in the CHILD process (dies with the agent).
 *
 * Pipe `query()` yields through `write()` + `sendResult()`. Read
 * `inboundPrompts()` (user typed on APEX.ai) into `query()`'s input
