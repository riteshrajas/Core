...
if (typeof MACRO === 'undefined') {
  (globalThis as any).MACRO = {
    ...
    PACKAGE_URL: 'APEX-code-source-snapshot',
...
if (process.env.APEX_CODE_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.APEX_CODE_ABLATION_BASELINE) {
  for (const k of ['APEX_CODE_SIMPLE', 'APEX_CODE_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'APEX_CODE_DISABLE_AUTO_MEMORY', 'APEX_CODE_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (APEX Code)`);
    return;
  }
  ...
  if (process.argv[2] === '--APEX-in-chrome-mcp') {
    profileCheckpoint('cli_APEX_in_chrome_mcp_path');
    const {
      runAPEXInChromeMcpServer
    } = await import('../utils/APEXInChrome/mcpServer.js');
    await runAPEXInChromeMcpServer();
    return;
  } else if (process.argv[2] === '--chrome-native-host') {
    profileCheckpoint('cli_chrome_native_host_path');
    const {
      runChromeNativeHost
    } = await import('../utils/APEXInChrome/chromeNativeHost.js');
  ...
  // Fast-path for `APEX remote-control` (also accepts legacy `APEX remote` / `APEX sync` / `APEX bridge`):
  ...
  // Fast-path for `APEX daemon [subcommand]`: long-running supervisor.
  ...
  // Fast-path for `APEX ps|logs|attach|kill` and `--bg`/`--background`.
  // Session management against the ~/.APEX/sessions/ registry. Flag
  ...
  // Fast-path for `APEX environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  ...
  // Fast-path for `APEX self-hosted-runner`: headless self-hosted-runner
