...
export const ThinkingAdaptiveSchema = lazySchema(() =>
  z
    ...
    .describe('APEX decides when and how much to think (Opus 4.6+).'),
)
...
export const ThinkingConfigSchema = lazySchema(() =>
  z
    ...
    .describe(
      "Controls APEX's thinking/reasoning behavior. When set, takes precedence over the deprecated maxThinkingTokens.",
    ),
)
...
export const McpAPEXAIProxyServerConfigSchema = lazySchema(() =>
  z.object({
    type: z.literal('APEXai-proxy'),
    url: z.string(),
    id: z.string(),
  }),
)

// Broader config type for status responses (includes APEXai-proxy which is output-only)
export const McpServerStatusConfigSchema = lazySchema(() =>
  z.union([
    McpServerConfigForProcessTransportSchema(),
    McpAPEXAIProxyServerConfigSchema(),
  ]),
)
  ...
  z
    .object({
      ...
      scope: z
        ...
        .optional()
        .describe(
          'Configuration scope (e.g., project, user, local, APEXai, managed)',
        ),
      tools: z
        .array(
          z.object({
      ...
      capabilities: z
        ...
        .optional()
        .describe(
          "@internal Server capabilities (available when connected). experimental['APEX/channel'] is only present if the server's plugin is on the approved channels allowlist — use its presence to decide whether to show an Enable-channel prompt.",
        ),
    })
  ...
  z
    .object({
      ...
      supportsAdaptiveThinking: z
        ...
        .optional()
        .describe(
          'Whether this model supports adaptive thinking (APEX decides when and how much to think)',
        ),
      supportsFastMode: z
        .boolean()
        .optional()
...
export const AccountInfoSchema = lazySchema(() =>
  z
    .object({
      ...
      apiProvider: z
        .enum(['firstParty', 'bedrock', 'vertex', 'foundry', '9router'])
        .optional()
        .describe(
          'Active API backend. Anthropic OAuth login only applies when "firstParty"; for 3P providers the other fields are absent and auth is external (AWS creds, gcloud ADC, local gateway, etc.).',
        ),
    })
  ...
  z
    .object({
      ...
      model: z
        ...
        .optional()
        .describe(
          "Model alias (e.g. 'sonnet', 'opus', 'haiku') or full model ID (e.g. 'APEX-opus-4-5'). If omitted or 'inherit', uses the main model",
        ),
      ...
      memory: z
        ...
        .optional()
        .describe(
          "Scope for auto-loading agent memory files. 'user' - ~/.APEX/agent-memory/<agentType>/, 'project' - .APEX/agent-memory/<agentType>/, 'local' - .APEX/agent-memory-local/<agentType>/",
        ),
      effort: z
  ...
  z
    ...
    .describe(
      'Source for loading filesystem-based settings. ' +
        "'user' - Global user settings (~/.APEX/settings.json). " +
        "'project' - Project settings (.APEX/settings.json). " +
        "'local' - Local settings (.APEX/settings.local.json).",
    ),
)
...
export const SDKRateLimitInfoSchema = lazySchema(() =>
  z
    ...
    .describe('Rate limit information for APEX.ai subscription users.'),
)
