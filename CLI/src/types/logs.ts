...
export type SerializedMessage = Message & {
  cwd: string
  userType: string
  entrypoint?: string // APEX_CODE_ENTRYPOINT — distinguishes cli/sdk-ts/sdk-py/etc.
  sessionId: string
  timestamp: string
  version: string
  gitBranch?: string
 ...
 * `APEX ps` can show something more useful than the last user prompt
...
/**
 * Per-file attribution state tracking APEX's character contributions.
 */
export type FileAttributionState = {
  contentHash: string // SHA-256 hash of file content
  APEXContribution: number // Characters written by APEX
  mtime: number // File modification time
}

/**
 * Attribution snapshot message stored in session transcript.
 * Tracks character-level contributions by APEX for commit attribution.
 */
