...
import { getAPEXConfigHomeDir } from 'src/utils/envUtils.js';
...
export function Doctor(t0) {
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    const envVars = [{
      ...
      name: "APEX_CODE_MAX_OUTPUT_TOKENS",
      ...getModelMaxOutputTokens("APEX-opus-4-6")
    }];
  ...
  if ($[6] !== agentDefinitions || $[7] !== toolPermissionContext || $[8] !== tools) {
    t5 = () => {
      ...
      (async () => {
        const userAgentsDir = join(getAPEXConfigHomeDir(), "agents");
        const projectAgentsDir = join(getOriginalCwd(), ".APEX", "agents");
        const {
          activeAgents,
          allAgents,
          failedFiles
        ...
        if (isPidBasedLockingEnabled()) {
          const locksDir = join(getXDGStateHome(), "APEX", "locks");
  ...
  if ($[11] !== onDone) {
    t7 = () => {
      onDone("APEX Code diagnostics dismissed", {
        display: "system"
      });
    };
    $[11] = onDone;
