...
export function getMacOsKeychainStorageServiceName(
  ...
  const isDefaultDir = !process.env.APEX_CONFIG_DIR
  ...
  return `APEX Code${getOauthConfig().OAUTH_FILE_SUFFIX}${serviceSuffix}${dirHash}`
}
...
//
// The sync read() path takes ~500ms per `security` spawn. With 50+ APEX.ai
