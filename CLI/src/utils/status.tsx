...
import { getAccountInformation, isAPEXAISubscriber } from './auth.js';
import { getLargeMemoryFiles, getMemoryFiles, MAX_MEMORY_CHARACTER_COUNT } from './APEXmd.js';
...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (isEnvTruthy(process.env.APEX_CODE_SKIP_BEDROCK_AUTH)) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.APEX_CODE_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.APEX_CODE_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.APEX_CODE_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.APEX_CODE_CLIENT_KEY
      });
    }
  }
  return properties;
...
export function getModelDisplayLabel(mainLoopModel: string | null): string {
  ...
  if (mainLoopModel === null && isAPEXAISubscriber()) {
    const description = getAPEXAiUserDefaultModelDescription();
