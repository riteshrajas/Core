...
export const getGlobalAPEXFile = memoize((): string => {
  // Legacy fallback for backwards compatibility
  if (
    getFsImplementation().existsSync(
      join(getAPEXConfigHomeDir(), '.config.json'),
    )
  ) {
    return join(getAPEXConfigHomeDir(), '.config.json')
  }

  const filename = `.APEX${fileSuffixForOauthConfig()}.json`
  return join(process.env.APEX_CONFIG_DIR || homedir(), filename)
})
 ...
 * If APEX_CODE_HOST_PLATFORM is set to a valid platform value, that overrides
...
export function getHostPlatformForAnalytics(): Platform {
  const override = process.env.APEX_CODE_HOST_PLATFORM
