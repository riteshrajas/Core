 ...
 * - Checks environment kind (APEX_CODE_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires APEX_CODE_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.APEX_CODE_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but APEX_CODE_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
