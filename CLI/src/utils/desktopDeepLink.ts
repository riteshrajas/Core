...
/**
 * Builds a deep link URL for APEX Desktop to resume a CLI session.
 * Format: APEX://resume?session={sessionId}&cwd={cwd}
 * In dev mode: APEX-dev://resume?session={sessionId}&cwd={cwd}
 */
function buildDesktopDeepLink(sessionId: string): string {
  const protocol = isDevMode() ? 'APEX-dev' : 'APEX'
...
/**
 * Check if APEX Desktop app is installed.
 * On macOS, checks for /Applications/APEX.app.
 * On Linux, checks if xdg-open can handle APEX:// protocol.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for APEX.app in /Applications
    return pathExists('/Applications/APEX.app')
  } else if (platform === 'linux') {
    // Check if xdg-mime can find a handler for APEX://
...
/**
 * Detect the installed APEX Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/APEX.app/Contents/Info.plist',
    ...
    const installDir = join(localAppData, 'AnthropicAPEX')
    try {
...
/**
 * Build and open a deep link to resume the current session in APEX Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'APEX Desktop is not installed. Install it from https://APEX.ai/download',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open APEX Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
