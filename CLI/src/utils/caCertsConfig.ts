 ...
 * user-controlled files (~/.APEX/settings.json and ~/.APEX.json),
 ...
 *
 * We read from global config (~/.APEX.json) and user settings
 * (~/.APEX/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.APEX/settings.json),
