...
export async function getAPEXDesktopConfigPath(): Promise<string> {
  const platform = getPlatform()

  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - APEX Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/APEX/APEX_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for APEX Desktop config in each user directory
  ...
  throw new Error(
    'Could not find APEX Desktop config file in Windows. Make sure APEX Desktop is installed on Windows.',
  )
}

export async function readAPEXDesktopMcpServers(): Promise<
  Record<string, McpServerConfig>
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - APEX Desktop integration only works on macOS and WSL.',
    )
  }
  try {
    const configPath = await getAPEXDesktopConfigPath()
