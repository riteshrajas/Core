 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches APEX Code
...
/**
 * Launch APEX Code in the detected terminal emulator.
 *
 ...
 *
 * For pure-argv paths: APEXPath, --prefill, query, cwd travel as distinct
 * argv elements end-to-end. No sh -c. No shellQuote(). The terminal does
 * chdir(cwd) and execvp(APEX, argv). Spaces/quotes/metacharacters in
...
export async function launchInTerminal(
  ...
  const APEXArgs = ['--deep-link-origin']
  if (action.repo) {
    APEXArgs.push('--deep-link-repo', action.repo)
    if (action.lastFetchMs !== undefined) {
      APEXArgs.push('--deep-link-last-fetch', String(action.lastFetchMs))
    }
  }
  if (action.query) {
    APEXArgs.push('--prefill', action.query)
  }

  switch (process.platform) {
    case 'darwin':
      return launchMacosTerminal(terminal, APEXPath, APEXArgs, action.cwd)
    case 'linux':
      return launchLinuxTerminal(terminal, APEXPath, APEXArgs, action.cwd)
    case 'win32':
      return launchWindowsTerminal(terminal, APEXPath, APEXArgs, action.cwd)
    default:
      return false
  }
}
...
async function launchMacosTerminal(
  ...
  switch (terminal.command) {
    ...
    case 'iTerm': {
      const shCmd = buildShellCommand(APEXPath, APEXArgs, cwd)
...
async function launchWindowsTerminal(
  ...
  switch (terminal.name) {
    ...
    // mode that also keeps the session interactive after APEX exits.
    ...
    case 'PowerShell': {
      ...
      args.push(
        '-NoExit',
        '-Command',
        `${cdCmd}& ${psQuote(APEXPath)} ${APEXArgs.map(psQuote).join(' ')}`,
      )
      break
    }
    ...
    default: {
      ...
      args.push(
        '/k',
        `${cdCmd}${cmdQuote(APEXPath)} ${APEXArgs.map(a => cmdQuote(a)).join(' ')}`,
      )
      break
    }
  }
...
function buildShellCommand(
  ...
  return `${cdPrefix}${[APEXPath, ...APEXArgs].map(shellQuote).join(' ')}`
}
 ...
 * *child process* (APEX.exe) uses CommandLineToArgvW, where a trailing
