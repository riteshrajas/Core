 ...
 * APEX-cli:// deep link. Linux xdg-open and browsers with "always allow"
 * set dispatch the link with no OS-level confirmation, so the application
 * provides its own provenance signal — mirroring APEX.ai's security
 ...
 * notice which directory — and therefore which APEX.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which APEX.md
 ...
 * and whether its APEX.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — APEX.md may be stale' : ''}`,
    )
  }
