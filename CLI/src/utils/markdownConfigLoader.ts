...
// APEX configuration directory names
export const APEX_CONFIG_DIRECTORIES = [
  'commands',
  'agents',
  'output-styles',
  'skills',
...
export type APEXConfigDirectory = (typeof APEX_CONFIG_DIRECTORIES)[number]
 ...
 * See: https://github.com/anthropics/APEX-code/issues/13893
 *
 ...
 * stopping there makes the parent project's `.APEX/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.APEX/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .APEX directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.APEX/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .APEX/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  subdir: APEXConfigDirectory,
  cwd: string,
): string[] {
  ...
  while (true) {
    ...
    const APEXSubdir = join(current, '.APEX', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const userDir = join(getAPEXConfigHomeDir(), subdir)
    const managedDir = join(getManagedFilePath(), '.APEX', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .APEX/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .APEX/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .APEX/<subdir> content — loading the main
    // repo's copy too would duplicate every command/agent/skill
    // (anthropics/APEX-code#29599, #28182, #26992).
    //
    ...
    if (gitRoot && canonicalRoot && canonicalRoot !== gitRoot) {
      const worktreeSubdir = normalizePathForComparison(
        join(gitRoot, '.APEX', subdir),
      )
      const worktreeHasSubdir = projectDirs.some(
        dir => normalizePathForComparison(dir) === worktreeSubdir,
      )
      if (!worktreeHasSubdir) {
        const mainAPEXSubdir = join(canonicalRoot, '.APEX', subdir)
        if (!projectDirs.includes(mainAPEXSubdir)) {
          projectDirs.push(mainAPEXSubdir)
        }
      }
    }
    ...
    // This prevents the same file from appearing multiple times when ~/.APEX is
  ...
  (subdir: APEXConfigDirectory, cwd: string) => `${subdir}:${cwd}`,
)
 ...
 * 3. Can be explicitly enabled via APEX_CODE_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
 ...
 * @param dir Directory (eg. "~/.APEX/commands")
 * @returns Array of parsed markdown files with metadata
 */
...
> {
  ...
  // - Fallback: native Node.js (when APEX_CODE_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.APEX_CODE_USE_NATIVE_FILE_SEARCH)
