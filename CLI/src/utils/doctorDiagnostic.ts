...
async function getInstallationPath(): Promise<string> {
  ...
  if (isInBundledMode()) {
    ...
    try {
      await getFsImplementation().stat(join(homedir(), '.local/bin/APEX'))
      return join(homedir(), '.local/bin/APEX')
    } catch {
      // Not found
    }
    return 'native'
...
> {
  ...
  const localPath = join(homedir(), '.APEX', 'local')
  if (await localInstallationExists()) {
    installations.push({ type: 'npm-local', path: localPath })
  }

  // Check for global npm installation
  const packagesToCheck = ['@anthropic-ai/APEX-code']
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    // First check for active installations via bin/APEX
    // Linux / macOS have prefix/bin/APEX and prefix/lib/node_modules
    // Windows has prefix/APEX and prefix/node_modules
    const globalBinPath = isWindows
      ? join(npmPrefix, 'APEX')
      : join(npmPrefix, 'bin', 'APEX')
    ...
    if (globalBinExists) {
      ...
      // When npm is installed via Homebrew, both can exist at /opt/homebrew/bin/APEX
      ...
      // If no bin/APEX exists, check for orphaned packages (no bin/APEX symlink)
  ...
  const nativeBinPath = join(homedir(), '.local', 'bin', 'APEX')
  try {
  ...
  if (config.installMethod === 'native') {
    const nativeDataPath = join(homedir(), '.local', 'share', 'APEX')
    try {
...
async function detectConfigurationIssues(
  ...
  if (!isEnvTruthy(process.env.DISABLE_INSTALLATION_CHECKS)) {
    if (type === 'npm-local' && config.installMethod !== 'local') {
      warnings.push({
        ...
        fix: 'Consider using native installation: APEX install',
      })
    }
    ...
    if (type === 'native' && config.installMethod !== 'native') {
      warnings.push({
        ...
        fix: 'Run APEX install to update configuration',
      })
    }
  }
  ...
  const existingAlias = await findAPEXAlias()
  const validAlias = await findValidAPEXAlias()

  // Check if running local installation but it's not in PATH
  if (type === 'npm-local') {
    // Check if APEX is already accessible via PATH
    const whichResult = await which('APEX')
    const APEXInPath = !!whichResult

    // Only show warning if APEX is NOT in PATH AND no valid alias exists
    if (!APEXInPath && !validAlias) {
      if (existingAlias) {
        // Alias exists but points to invalid target
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias APEX="~/.APEX/local/APEX"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: 'Create alias: alias APEX="~/.APEX/local/APEX"',
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @anthropic-ai/APEX-code'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@anthropic-ai/APEX-code'
        ) {
  ...
  if (installationType === 'npm-global') {
    ...
    if (!hasUpdatePermissions && !getAutoUpdaterDisabledReason()) {
      warnings.push({
        ...
        fix: 'Do one of: (1) Re-install node without sudo, or (2) Use `APEX install` for native installation',
      })
    }
  }
