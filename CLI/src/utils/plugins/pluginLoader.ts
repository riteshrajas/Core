 ...
 *
 * This module is responsible for discovering, loading, and validating APEX Code plugins
 ...
 * Format: ~/.APEX/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.APEX/plugins/cache/{plugin-name}/
 *
...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.APEX_CODE_REMOTE)
 ...
 * APEX_CODE_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.APEX-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".APEX-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".APEX-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.APEX-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".APEX-plugin/name")
 ...
 *
 * APEX_CODE_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 * mode explicitly opts into blocking install before first query, and
 * main.tsx's getAPEXCodeMcpConfigs()/getInitialSettings().agent run
...
export const loadAllPluginsCacheOnly = memoize(
  async (): Promise<PluginLoadResult> => {
    if (isEnvTruthy(process.env.APEX_CODE_SYNC_PLUGIN_INSTALL)) {
 ...
 * - After modifying .APEX-plugin/ directory (for export)
