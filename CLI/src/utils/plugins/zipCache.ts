 ...
 * When APEX_CODE_PLUGIN_USE_ZIP_CACHE is enabled and APEX_CODE_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.APEX_CODE_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires APEX_CODE_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.APEX_CODE_PLUGIN_CACHE_DIR
...
export async function getSessionPluginCachePath(): Promise<string> {
  ...
  if (!sessionPluginCachePromise) {
    sessionPluginCachePromise = (async () => {
      ...
      const dir = join(tmpdir(), `APEX-plugin-session-${suffix}`)
...
async function collectFilesForZip(
  ...
  // See also: markdownConfigLoader.ts getFileIdentity, anthropics/APEX-code#13893
  try {
