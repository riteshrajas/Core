 ...
 * - CLI flag: --cowork
 * - Environment variable: APEX_CODE_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via APEX_CODE_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable APEX_CODE_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.APEX_CODE_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. APEX_CODE_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.APEX/plugins or ~/.APEX/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when APEX_CODE_PLUGIN_CACHE_DIR is set via settings.json
  // `env` (not shell), ~ is not expanded by the shell. Without this, a value
  // like "~/.APEX/plugins" becomes a literal `~` directory created in the
  // cwd of every project (gh-30794 / CC-212).
  const envOverride = process.env.APEX_CODE_PLUGIN_CACHE_DIR
  if (envOverride) {
    return expandTilde(envOverride)
  }
  return join(getAPEXConfigHomeDir(), getPluginsDirectoryName())
}
 ...
 * image and point APEX_CODE_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.APEX_CODE_PLUGIN_SEED_DIR
  if (!raw) return []
 ...
 * ${APEX_PLUGIN_DATA}. Unlike the version-scoped install cache
 * (${APEX_PLUGIN_ROOT}, which is orphaned and GC'd on every update),
 ...
 * .replace() so this isn't invoked unless ${APEX_PLUGIN_DATA} is present
