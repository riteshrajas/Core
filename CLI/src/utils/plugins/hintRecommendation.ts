 ...
 * `<APEX-code-hint />` tag to stderr (detected by the Bash/PowerShell tools).
 *
 * State persists in GlobalConfig.APEXCodeHints — a show-once record per
...
/**
 * Hard cap on `APEXCodeHints.plugin[]` — bounds config growth. Each shown
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.APEXCodeHints?.plugin ?? []
    if (existing.includes(pluginId)) return current
    return {
      ...current,
      APEXCodeHints: {
        ...current.APEXCodeHints,
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.APEXCodeHints?.disabled) return current
    return {
      ...current,
      APEXCodeHints: { ...current.APEXCodeHints, disabled: true },
    }
  })
}
