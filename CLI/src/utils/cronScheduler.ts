// Non-React scheduler core for .APEX/scheduled_tasks.json.
...
type CronSchedulerOptions = {
  ...
  /**
   * Directory containing .APEX/scheduled_tasks.json. When provided, the
...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    // and fires immediately). This avoids a misleading "missed while APEX
  ...
  function check() {
    ...
    // exists to stop two APEX sessions in the same cwd from double-firing
  ...
  async function enable() {
    ...
    // dies. Prevents double-firing when multiple APEXs share a cwd.
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while APEX was not running. ` +
    `${plural ? 'They have' : 'It has'} already been removed from .APEX/scheduled_tasks.json.\n\n` +
