/**
 * MDM (Mobile Device Management) profile enforcement for APEX Code managed settings.
 *
 * Reads enterprise settings from OS-level MDM configuration:
 * - macOS: `com.anthropic.APEXcode` preference domain
 *   (MDM profiles at /Library/Managed Preferences/ only — not user-writable ~/Library/Preferences/)
 * - Windows: `HKLM\SOFTWARE\Policies\APEXCode` (admin-only)
 *   and `HKCU\SOFTWARE\Policies\APEXCode` (user-writable, lowest priority)
 * - Linux: No MDM equivalent (uses /etc/APEX-code/managed-settings.json instead)
 *
