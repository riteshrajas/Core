...
function detectShell(): ShellInfo | null {
  ...
  const home = homedir()
  const APEXDir = join(home, '.APEX')

  if (shell.endsWith('/zsh') || shell.endsWith('/zsh.exe')) {
    const cacheFile = join(APEXDir, 'completion.zsh')
    return {
      name: 'zsh',
  ...
  if (shell.endsWith('/bash') || shell.endsWith('/bash.exe')) {
    const cacheFile = join(APEXDir, 'completion.bash')
    return {
      name: 'bash',
  ...
  if (shell.endsWith('/fish') || shell.endsWith('/fish.exe')) {
    ...
    const cacheFile = join(APEXDir, 'completion.fish')
    return {
      name: 'fish',
...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: APEX completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const APEXBin = process.argv[1] || 'APEX'
  const result = await execFileNoThrow(APEXBin, [
    'completion',
    shell.shellFlag,
    '--output',
    shell.cacheFile,
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: APEX completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes('APEX completion') ||
...
/**
 * Regenerate cached shell completion scripts in ~/.APEX/.
 * Called after `APEX update` so completions stay in sync with the new binary.
 */
