import type { ModelName } from './model.js'
import type { APIProvider } from './providers.js'

export type ModelConfig = Record<APIProvider, ModelName>

// @[MODEL LAUNCH]: Add a new APEX_*_CONFIG constant here. Double check the correct model strings
// here since the pattern may change.

export const APEX_3_7_SONNET_CONFIG = {
  firstParty: 'APEX-3-7-sonnet-20250219',
  bedrock: 'us.anthropic.APEX-3-7-sonnet-20250219-v1:0',
  vertex: 'APEX-3-7-sonnet@20250219',
  foundry: 'APEX-3-7-sonnet',
  openai: 'APEX-3-7-sonnet-20250219',
} as const satisfies ModelConfig

export const APEX_3_5_V2_SONNET_CONFIG = {
  firstParty: 'APEX-3-5-sonnet-20241022',
  bedrock: 'anthropic.APEX-3-5-sonnet-20241022-v2:0',
  vertex: 'APEX-3-5-sonnet-v2@20241022',
  foundry: 'APEX-3-5-sonnet',
  openai: 'APEX-3-5-sonnet-20241022',
} as const satisfies ModelConfig

export const APEX_3_5_HAIKU_CONFIG = {
  firstParty: 'APEX-3-5-haiku-20241022',
  bedrock: 'us.anthropic.APEX-3-5-haiku-20241022-v1:0',
  vertex: 'APEX-3-5-haiku@20241022',
  foundry: 'APEX-3-5-haiku',
  openai: 'APEX-3-5-haiku-20241022',
} as const satisfies ModelConfig

export const APEX_HAIKU_4_5_CONFIG = {
  firstParty: 'APEX-haiku-4-5-20251001',
  bedrock: 'us.anthropic.APEX-haiku-4-5-20251001-v1:0',
  vertex: 'APEX-haiku-4-5@20251001',
  foundry: 'APEX-haiku-4-5',
  openai: 'APEX-haiku-4-5-20251001',
} as const satisfies ModelConfig

export const APEX_SONNET_4_CONFIG = {
  firstParty: 'APEX-sonnet-4-20250514',
  bedrock: 'us.anthropic.APEX-sonnet-4-20250514-v1:0',
  vertex: 'APEX-sonnet-4@20250514',
  foundry: 'APEX-sonnet-4',
  openai: 'APEX-sonnet-4-20250514',
} as const satisfies ModelConfig

export const APEX_SONNET_4_5_CONFIG = {
  firstParty: 'APEX-sonnet-4-5-20250929',
  bedrock: 'us.anthropic.APEX-sonnet-4-5-20250929-v1:0',
  vertex: 'APEX-sonnet-4-5@20250929',
  foundry: 'APEX-sonnet-4-5',
  openai: 'APEX-sonnet-4-5-20250929',
} as const satisfies ModelConfig

export const APEX_OPUS_4_CONFIG = {
  firstParty: 'APEX-opus-4-20250514',
  bedrock: 'us.anthropic.APEX-opus-4-20250514-v1:0',
  vertex: 'APEX-opus-4@20250514',
  foundry: 'APEX-opus-4',
  openai: 'APEX-opus-4-20250514',
} as const satisfies ModelConfig

export const APEX_OPUS_4_1_CONFIG = {
  firstParty: 'APEX-opus-4-1-20250805',
  bedrock: 'us.anthropic.APEX-opus-4-1-20250805-v1:0',
  vertex: 'APEX-opus-4-1@20250805',
  foundry: 'APEX-opus-4-1',
  openai: 'APEX-opus-4-1-20250805',
} as const satisfies ModelConfig

export const APEX_OPUS_4_5_CONFIG = {
  firstParty: 'APEX-opus-4-5-20251101',
  bedrock: 'us.anthropic.APEX-opus-4-5-20251101-v1:0',
  vertex: 'APEX-opus-4-5@20251101',
  foundry: 'APEX-opus-4-5',
  openai: 'APEX-opus-4-5-20251101',
} as const satisfies ModelConfig

export const APEX_OPUS_4_6_CONFIG = {
  firstParty: 'APEX-opus-4-6',
  bedrock: 'us.anthropic.APEX-opus-4-6-v1',
  vertex: 'APEX-opus-4-6',
  foundry: 'APEX-opus-4-6',
  openai: 'APEX-opus-4-6',
} as const satisfies ModelConfig

export const APEX_SONNET_4_6_CONFIG = {
  firstParty: 'APEX-sonnet-4-6',
  bedrock: 'us.anthropic.APEX-sonnet-4-6',
  vertex: 'APEX-sonnet-4-6',
  foundry: 'APEX-sonnet-4-6',
  openai: 'APEX-sonnet-4-6',
} as const satisfies ModelConfig

// OpenAI Codex models
export const GPT_5_4_CONFIG = {
  firstParty: 'gpt-5.4',
  bedrock: 'gpt-5.4',
  vertex: 'gpt-5.4',
  foundry: 'gpt-5.4',
  openai: 'gpt-5.4',
} as const satisfies ModelConfig

export const GPT_5_3_CODEX_CONFIG = {
  firstParty: 'gpt-5.3-codex',
  bedrock: 'gpt-5.3-codex',
  vertex: 'gpt-5.3-codex',
  foundry: 'gpt-5.3-codex',
  openai: 'gpt-5.3-codex',
} as const satisfies ModelConfig

export const GPT_5_4_MINI_CONFIG = {
  firstParty: 'gpt-5.4-mini',
  bedrock: 'gpt-5.4-mini',
  vertex: 'gpt-5.4-mini',
  foundry: 'gpt-5.4-mini',
  openai: 'gpt-5.4-mini',
} as const satisfies ModelConfig

// @[MODEL LAUNCH]: Register the new config here.
export const ALL_MODEL_CONFIGS = {
  haiku35: APEX_3_5_HAIKU_CONFIG,
  haiku45: APEX_HAIKU_4_5_CONFIG,
  sonnet35: APEX_3_5_V2_SONNET_CONFIG,
  sonnet37: APEX_3_7_SONNET_CONFIG,
  sonnet40: APEX_SONNET_4_CONFIG,
  sonnet45: APEX_SONNET_4_5_CONFIG,
  sonnet46: APEX_SONNET_4_6_CONFIG,
  opus40: APEX_OPUS_4_CONFIG,
  opus41: APEX_OPUS_4_1_CONFIG,
  opus45: APEX_OPUS_4_5_CONFIG,
  opus46: APEX_OPUS_4_6_CONFIG,
  // OpenAI Codex models
  gpt54: GPT_5_4_CONFIG,
  gpt53codex: GPT_5_3_CODEX_CONFIG,
  gpt54mini: GPT_5_4_MINI_CONFIG,
} as const satisfies Record<string, ModelConfig>

export type ModelKey = keyof typeof ALL_MODEL_CONFIGS

/** Union of all canonical first-party model IDs, e.g. 'APEX-opus-4-6' | 'APEX-sonnet-4-5-20250929' | … */
export type CanonicalModelId =
  (typeof ALL_MODEL_CONFIGS)[ModelKey]['firstParty']

/** Runtime list of canonical model IDs — used by comprehensiveness tests. */
export const CANONICAL_MODEL_IDS = Object.values(ALL_MODEL_CONFIGS).map(
  c => c.firstParty,
) as [CanonicalModelId, ...CanonicalModelId[]]

/** Map canonical ID → internal short key. Used to apply settings-based modelOverrides. */
export const CANONICAL_ID_TO_KEY: Record<CanonicalModelId, ModelKey> =
  Object.fromEntries(
    (Object.entries(ALL_MODEL_CONFIGS) as [ModelKey, ModelConfig][]).map(
      ([key, cfg]) => [cfg.firstParty, key],
    ),
  ) as Record<CanonicalModelId, ModelKey>
