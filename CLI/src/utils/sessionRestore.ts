...
import { clearMemoryFileCaches } from './APEXmd.js'
import {
 ...
 * Used for computing initial state before render (per APEX.md guidelines).
...
export async function processResumedConversation(
  ...
  if (!opts.forkSession) {
    ...
    // the pre-loaded records). Without this seed, `APEX -r {newSessionId}`
  ...
  // Compute initial state before render (per APEX.md guidelines)
