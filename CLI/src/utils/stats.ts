...
/**
 * Convert a PersistedStatsCache to APEXCodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all APEX Code sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateAPEXCodeStats(): Promise<APEXCodeStats> {
 ...
 */
export async function aggregateAPEXCodeStatsForRange(
  range: StatsDateRange,
): Promise<APEXCodeStats> {
  if (range === 'all') {
    return aggregateAPEXCodeStats()
  }
  ...
  return processedStatsToAPEXCodeStats(stats)
}

/**
 * Convert ProcessedStats to APEXCodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToAPEXCodeStats(
...
function getEmptyStats(): APEXCodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
