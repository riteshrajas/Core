 ...
 *
 * This module provides PID-based locking for running APEX Code versions.
...
/**
 * Validate that a running process is actually a APEX process
 * This helps mitigate PID reuse issues
 */
function isAPEXProcess(pid: number, expectedExecPath: string): boolean {
  if (!isProcessRunning(pid)) {
    return false
  }

  // If the PID matches our current process, we know it's valid
  // This handles test environments where the command might not contain 'APEX'
  if (pid === process.pid) {
    return true
  }
  ...
  try {
    ...
    // Check if the command contains 'APEX' or the expected exec path
    const normalizedCommand = command.toLowerCase()
    const normalizedExecPath = expectedExecPath.toLowerCase()

    return (
      normalizedCommand.includes('APEX') ||
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a APEX process?
  // This helps with PID reuse scenarios
  if (!isAPEXProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be APEX - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a APEX process (to handle PID reuse scenarios)
