/**
 * APEX Code hints protocol.
 *
 * CLIs and SDKs running under APEX Code can emit a self-closing
 * `<APEX-code-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/APEX-code-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type APEXCodeHintType = 'plugin'

export type APEXCodeHint = {
  /** Spec version declared by the emitter. Unknown versions are dropped. */
  v: number
  /** Hint discriminator. v1 defines only `plugin`. */
  type: APEXCodeHintType
  /**
 ...
 */
const HINT_TAG_RE = /^[ \t]*<APEX-code-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
 ...
 */
export function extractAPEXCodeHints(
  output: string,
  command: string,
): { hints: APEXCodeHint[]; stripped: string } {
  // Fast path: no tag open sequence → no work, no allocation.
  if (!output.includes('<APEX-code-hint')) {
    return { hints: [], stripped: output }
  }

  const sourceCommand = firstCommandToken(command)
  const hints: APEXCodeHint[] = []
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[APEXCodeHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[APEXCodeHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[APEXCodeHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as APEXCodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: APEXCodeHint | null = null
...
export function setPendingHint(hint: APEXCodeHint): void {
...
export function getPendingHintSnapshot(): APEXCodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetAPEXCodeHintStore(): void {
  pendingHint = null
