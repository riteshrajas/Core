 ...
 *
 * 1. Managed memory (eg. /etc/APEX-code/APEX.md) - Global instructions for all users
 * 2. User memory (~/.APEX/APEX.md) - Private global instructions for all projects
 * 3. Project memory (APEX.md, .APEX/APEX.md, and .APEX/rules/*.md in project roots) - Instructions checked into the codebase
 * 4. Local memory (APEX.local.md in project roots) - Private project-specific instructions
 *
 ...
 * - APEX.md, .APEX/APEX.md, and all .md files in .APEX/rules/ are checked in each directory for Project memory
 *
...
import {
  getAdditionalDirectoriesForAPEXMd,
  getOriginalCwd,
...
import {
  ...
  getManagedAPEXRulesDir,
  getMemoryPath,
  getUserAPEXRulesDir,
} from './config.js'
import { logForDebugging } from './debug.js'
import { logForDiagnosticsNoPII } from './diagLogs.js'
import { getAPEXConfigHomeDir, isEnvTruthy } from './envUtils.js'
...
function handleMemoryFileReadError(error: unknown, filePath: string): void {
  ...
  if (code === 'EACCES') {
    ...
    logEvent('tengu_APEX_md_permission_error', {
      is_access_error: 1,
      has_home_dir: filePath.includes(getAPEXConfigHomeDir()) ? 1 : 0,
    })
  }
}
...
/**
 * Checks whether a APEX.md file path is excluded by the APEXMdExcludes setting.
 ...
 */
function isAPEXMdExcluded(filePath: string, type: MemoryType): boolean {
  if (type !== 'User' && type !== 'Project' && type !== 'Local') {
    return false
  }

  const patterns = getInitialSettings().APEXMdExcludes
  ...
  // the user writes "/tmp/project/APEX.md" in their exclude, but the system
...
function resolveExcludePatterns(patterns: string[]): string[] {
  ...
  for (const normalized of expanded) {
    ...
    try {
      // sync IO: called from sync context (isAPEXMdExcluded -> processMemoryFile -> getMemoryFiles)
...
export async function processMemoryFile(
  ...
  // Skip if path is excluded by APEXMdExcludes setting
  if (isAPEXMdExcluded(filePath, type)) {
    return []
  }
...
/**
 * Processes all .md files in the .APEX/rules/ directory and its subdirectories
...
export async function processMdRules({
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('EACCES')) {
      logEvent('tengu_APEX_rules_md_permission_error', {
        is_access_error: 1,
        has_home_dir: rulesDir.includes(getAPEXConfigHomeDir()) ? 1 : 0,
      })
    }
    return []
  }
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    const includeExternal =
      ...
      config.hasAPEXMdExternalIncludesApproved ||
      false

    // Process Managed file first (always loaded - policy settings)
    const managedAPEXMd = getMemoryPath('Managed')
    result.push(
    ...
    // Process Managed .APEX/rules/*.md files
    const managedAPEXRulesDir = getManagedAPEXRulesDir()
    result.push(
      ...(await processMdRules({
        rulesDir: managedAPEXRulesDir,
        type: 'Managed',
        processedPaths,
        includeExternal,
    ...
    if (isSettingSourceEnabled('userSettings')) {
      const userAPEXMd = getMemoryPath('User')
      result.push(
        ...(await processMemoryFile(
          userAPEXMd,
          'User',
      ...
      // Process User ~/.APEX/rules/*.md files
      const userAPEXRulesDir = getUserAPEXRulesDir()
      result.push(
        ...(await processMdRules({
          rulesDir: userAPEXRulesDir,
          type: 'User',
          processedPaths,
    ...
    // .APEX/worktrees/<name>/ from `APEX -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like APEX.md and .APEX/rules/*.md, so the same
    // content gets loaded twice. Skip Project-type (checked-in) files from
    // directories above the worktree but within the main repo — the worktree
    // already has its own checkout. APEX.local.md is gitignored so it only
    // exists in the main repo and is still loaded.
    // See: https://github.com/anthropics/APEX-code/issues/29599
    ...
    for (const dir of dirs.reverse()) {
      ...
      // Try reading APEX.md (Project) - only if projectSettings is enabled
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        const projectPath = join(dir, 'APEX.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
            'Project',
        ...
        // Try reading .APEX/APEX.md (Project)
        const dotAPEXPath = join(dir, '.APEX', 'APEX.md')
        result.push(
          ...(await processMemoryFile(
            dotAPEXPath,
            'Project',
        ...
        // Try reading .APEX/rules/*.md files (Project)
        const rulesDir = join(dir, '.APEX', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
      ...
      // Try reading APEX.local.md (Local) - only if localSettings is enabled
      if (isSettingSourceEnabled('localSettings')) {
        const localPath = join(dir, 'APEX.local.md')
        result.push(
          ...(await processMemoryFile(
            localPath,
            'Local',
    ...
    // Process APEX.md from additional directories (--add-dir) if env var is enabled
    // This is controlled by APEX_CODE_ADDITIONAL_DIRECTORIES_APEX_MD and defaults to off
    // Note: we don't check isSettingSourceEnabled('projectSettings') here because --add-dir
    // is an explicit user action and the SDK defaults settingSources to [] when not specified
    if (isEnvTruthy(process.env.APEX_CODE_ADDITIONAL_DIRECTORIES_APEX_MD)) {
      const additionalDirs = getAdditionalDirectoriesForAPEXMd()
      for (const dir of additionalDirs) {
        // Try reading APEX.md from the additional directory
        const projectPath = join(dir, 'APEX.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
        ...
        // Try reading .APEX/APEX.md from the additional directory
        const dotAPEXPath = join(dir, '.APEX', 'APEX.md')
        result.push(
          ...(await processMemoryFile(
            dotAPEXPath,
        ...
        // Try reading .APEX/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.APEX', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
    ...
    if (!hasLoggedInitialLoad) {
      ...
      logEvent('tengu_APEXmd__initial_load', {
        file_count: result.length,
        total_content_length: totalContentLength,
    ...
    // memory system, not "instructions" in the APEX.md/rules sense.
    // Gated on !forceIncludeExternal: the forceIncludeExternal=true variant
    // is only used by getExternalAPEXMdIncludes() for approval checks, not
...
export async function getManagedAndUserConditionalRules(
  ...
  // Process Managed conditional .APEX/rules/*.md files
  const managedAPEXRulesDir = getManagedAPEXRulesDir()
  result.push(
  ...
  if (isSettingSourceEnabled('userSettings')) {
    // Process User conditional .APEX/rules/*.md files
    const userAPEXRulesDir = getUserAPEXRulesDir()
    result.push(
      ...(await processConditionedMdRules(
        targetPath,
 ...
 * Loads APEX.md, unconditional rules, and conditional rules for that directory.
 *
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (APEX.md and .APEX/APEX.md)
  ...
  // Process local memory file (APEX.local.md)
  if (isSettingSourceEnabled('localSettings')) {
    const localPath = join(dir, 'APEX.local.md')
    result.push(
  ...
  // Process project unconditional .APEX/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .APEX/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
export async function processConditionedMdRules(
  ...
  return conditionedRuleMdFiles.filter(file => {
    ...
    // For Project rules: glob patterns are relative to the directory containing .APEX
    // For Managed/User rules: glob patterns are relative to the original CWD
    const baseDir =
      type === 'Project'
        ? dirname(dirname(rulesDir)) // Parent of .APEX
...
export type ExternalAPEXMdInclude = {
  path: string
  parent: string
}

export function getExternalAPEXMdIncludes(
  files: MemoryFileInfo[],
): ExternalAPEXMdInclude[] {
  const externals: ExternalAPEXMdInclude[] = []
...
export function hasExternalAPEXMdIncludes(files: MemoryFileInfo[]): boolean {
  return getExternalAPEXMdIncludes(files).length > 0
}

export async function shouldShowAPEXMdExternalIncludesWarning(): Promise<boolean> {
  const config = getCurrentProjectConfig()
  if (
    config.hasAPEXMdExternalIncludesApproved ||
    config.hasAPEXMdExternalIncludesWarningShown
  ) {
    return false
  }

  return hasExternalAPEXMdIncludes(await getMemoryFiles(true))
}

/**
 * Check if a file path is a memory file (APEX.md, APEX.local.md, or .APEX/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  const name = basename(filePath)

  // APEX.md or APEX.local.md anywhere
  if (name === 'APEX.md' || name === 'APEX.local.md') {
    return true
  }

  // .md files in .APEX/rules/ directories
  if (
    name.endsWith('.md') &&
    filePath.includes(`${sep}.APEX${sep}rules${sep}`)
  ) {
    return true
  }
