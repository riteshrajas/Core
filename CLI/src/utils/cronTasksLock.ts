// Scheduler lease lock for .APEX/scheduled_tasks.json.
//
// When multiple APEX sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.APEX', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .APEX/ doesn't exist yet — create it and retry once. In steady
