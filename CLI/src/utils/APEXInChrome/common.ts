...
export const APEX_IN_CHROME_MCP_SERVER_NAME = 'APEX-in-chrome'
      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[APEX in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isAPEXInChromeMCPServer(name: string): boolean {
  return normalizeNameForMCP(name) === APEX_IN_CHROME_MCP_SERVER_NAME
}

const MAX_TRACKED_TABS = 200
const trackedTabIds = new Set<number>()

export function trackAPEXInChromeTabId(tabId: number): void {
...
export function isTrackedAPEXInChromeTabId(tabId: number): boolean {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[APEX in Chrome] No compatible browser found')
    return false
  }
...
export function getSocketDir(): string {
  return `/tmp/APEX-mcp-browser-bridge-${getUsername()}`
}
...
export function getAllSocketPaths(): string[] {
  ...
  try {
    // eslint-disable-next-line custom-rules/no-sync-fs -- APEXForChromeContext.getSocketPaths (external @ant/APEX-for-chrome-mcp) requires a sync () => string[] callback
  ...
  const legacyName = `APEX-mcp-browser-bridge-${getUsername()}`
...
function getSocketName(): string {
  // NOTE: This must match the one used in the APEX in Chrome MCP
  return `APEX-mcp-browser-bridge-${getUsername()}`
}
