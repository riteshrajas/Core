...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.APEX/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.APEX/settings.json)'
    case 'projectSettings':
      return 'Project settings (.APEX/settings.json)'
    case 'localSettings':
      return 'Local settings (.APEX/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.APEX/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.APEX/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by APEX Code)'
    default:
