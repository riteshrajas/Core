 ...
 * socket (e.g., "APEX-panel-a1b2c3d4") holds the shell session. Each APEX
 ...
 * APEX Code while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per APEX Code instance (based on session ID)
...
export function getTerminalPanelSocket(): string {
  ...
  return `APEX-panel-${sessionId.slice(0, 8)}`
}
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to APEX Code from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to APEX ', ';',
