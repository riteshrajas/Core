...
/**
 * Detects if a file path is a session-related file under ~/.APEX.
...
/**
 * Check if a file is a APEX-managed memory file (NOT user-managed instruction files).
 * Includes: auto-memory (memdir), agent memory, session memory/transcripts.
 * Excludes: APEX.md, APEX.local.md, .APEX/rules/*.md (user-managed).
 *
...
export function isMemoryDirectory(dirPath: string): boolean {
  ...
  // Check the auto-memory path override (APEX_COWORK_MEMORY_PATH_OVERRIDE)
  if (isAutoMemoryEnabled()) {
  ...
  const configDirCmp = toComparable(getAPEXConfigHomeDir())
...
// Excludes APEX.md, APEX.local.md, .APEX/rules/ (user-managed).
