...
function getSessionsDir(): string {
  return join(getAPEXConfigHomeDir(), 'sessions')
}

/**
 * Kind override from env. Set by the spawner (`APEX --bg`, daemon
...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.APEX_CODE_SESSION_KIND
...
/**
 * True when this REPL is running inside a `APEX --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `APEX ps` sees everything
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.APEX_CODE_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.APEX_CODE_SESSION_NAME,
              logPath: process.env.APEX_CODE_SESSION_LOG,
              agent: process.env.APEX_CODE_AGENT,
            }
          : {}),
      }),
    )
    // --resume / /resume mutates getSessionId() via switchSession. Without
    // this, the PID file's sessionId goes stale and `APEX ps` sparkline
...
/**
 * Push live activity state for `APEX ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See anthropics/APEX-code#34210.
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.APEX/sessions/ is shared with Windows-native APEX (symlink
      // or APEX_CONFIG_DIR), a Windows PID won't be probeable from WSL
