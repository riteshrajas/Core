...
/**
 * Generates APEX Code specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getAPEXCodeSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in APEX's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and APEX Code content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const APEXCodeContent = await getAPEXCodeSnapshotContent()
 ...
 *
 * This function is a critical part of APEX CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows APEX CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with APEX Code defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getAPEXConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.APEX_CODE_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - APEX home: ${getAPEXConfigHomeDir()}`)
