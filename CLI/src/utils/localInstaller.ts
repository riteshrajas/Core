...
// hfi.tsx get a chance to set APEX_CONFIG_DIR in main(), and would also
// populate the memoize cache with that stale value for all 150+ other callers.
function getLocalInstallDir(): string {
  return join(getAPEXConfigHomeDir(), 'local')
}
export function getLocalAPEXPath(): string {
  return join(getLocalInstallDir(), 'APEX')
}
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.APEX/local/node_modules/')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'APEX-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const wrapperPath = join(localInstallDir, 'APEX')
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/APEX" "$@"`,
      0o755,
    )
    if (created) {
...
/**
 * Install or update APEX CLI package in the local directory
 ...
 */
export async function installOrUpdateAPEXPackage(
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install APEX CLI package: ${result.stderr}`,
      )
      logError(error)
...
export async function localInstallationExists(): Promise<boolean> {
  try {
    await access(join(getLocalInstallDir(), 'node_modules', '.bin', 'APEX'))
    return true
  } catch {
    return false
  }
