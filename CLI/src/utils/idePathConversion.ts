 ...
 * Handles conversions between APEX's environment and the IDE's environment
 */
...
export interface IDEPathConverter {
  /**
   * Convert path from IDE format to APEX's local format
   * Used when reading workspace folders from IDE lockfile
   */
  toLocalPath(idePath: string): string

  /**
   * Convert path from APEX's local format to IDE format
...
/**
 * Converter for Windows IDE + WSL APEX scenario
 */
