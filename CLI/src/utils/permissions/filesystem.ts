...
import {
  APEX_FOLDER_PERMISSION_PATTERN,
  FILE_EDIT_TOOL_NAME,
  GLOBAL_APEX_FOLDER_PERMISSION_PATTERN,
...
import { getAPEXConfigHomeDir } from '../envUtils.js'
import {
  getFsImplementation,
 ...
 * filesystems (macOS/Windows) like `.APEX/Settings.locaL.json`.
 *
...
/**
 * If filePath is inside a .APEX/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .APEX/ (settings.json, hooks/, etc.).
 */
export function getAPEXSkillScope(
  filePath: string,
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.APEX', 'skills')),
      prefix: '/.APEX/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.APEX', 'skills')),
      prefix: '~/.APEX/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.APEX/skills/*/**' which matches ALL skills. Return null
...
export function isAPEXSettingsPath(filePath: string): boolean {
  // SECURITY: Normalize path structure first to prevent bypass via redundant ./
  // sequences like `./.APEX/./settings.json` which would evade the endsWith() check
  const expandedPath = expandPath(filePath)

  // Normalize for case-insensitive comparison to prevent bypassing security
  // with paths like .APEX/Settings.locaL.json
  const normalizedPath = normalizeCaseForComparison(expandedPath)

  // Use platform separator so endsWith checks work on both Unix (/) and Windows (\)
  if (
    normalizedPath.endsWith(`${sep}.APEX${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.APEX${sep}settings.local.json`)
  ) {
    // Include .APEX/settings.json even for other projects
    return true
  }
...
// Always ask when APEX Code tries to edit its own config files
function isAPEXConfigFilePath(filePath: string): boolean {
  if (isAPEXSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .APEX/commands or .APEX/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.APEX', 'commands')
  const agentsDir = join(getOriginalCwd(), '.APEX', 'agents')
  const skillsDir = join(getOriginalCwd(), '.APEX', 'skills')
 ...
 * Path format: ~/.APEX/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for APEX to write temporary files.
...
/**
 * Returns the user-specific APEX temp directory name.
 * On Unix: 'APEX-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'APEX' (tmpdir() is already per-user)
 */
export function getAPEXTempDirName(): string {
  if (getPlatform() === 'windows') {
    return 'APEX'
  }
...
/**
 * Returns the APEX temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/APEX-{uid}/ (resolved to /private/tmp/APEX-{uid}/ on macOS)
 * - On Windows: {tmpdir}/APEX/ (e.g., C:\Users\{user}\AppData\Local\Temp\APEX\)
 * This is a per-user temporary directory used by APEX Code for all temp files.
 *
 ...
 * resolution, paths like /tmp/APEX-{uid}/... wouldn't match /private/tmp/APEX-{uid}/...
 */
// Memoized: called per-tool from permission checks (yoloClassifier, sandbox-adapter)
// and per-turn from BashTool prompt. Inputs (APEX_CODE_TMPDIR env + platform) are
// fixed at startup, and the realpath of the system tmp dir does not change mid-session.
export const getAPEXTempDir = memoize(function getAPEXTempDir(): string {
  const baseTmpDir =
    process.env.APEX_CODE_TMPDIR ||
  ...
  return join(resolvedBaseTmpDir, getAPEXTempDirName()) + sep
})
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getAPEXTempDir(), 'bundled-skills', MACRO.VERSION, nonce)
  },
)

/**
 * Returns the project temp directory path with trailing separator.
 * Path format: /tmp/APEX-{uid}/{sanitized-cwd}/
 */
export function getProjectTempDir(): string {
  return join(getAPEXTempDir(), sanitizePath(getOriginalCwd())) + sep
}

/**
 * Returns the scratchpad directory path for the current session.
 * Path format: /tmp/APEX-{uid}/{sanitized-cwd}/{sessionId}/scratchpad/
 */
...
function isScratchpadPath(absolutePath: string): boolean {
  ...
  //   echo "malicious" > /tmp/APEX-0/proj/session/scratchpad/../../../etc/passwd
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .APEX/worktrees/ is a structural path (where APEX stores
      // git worktrees), not a user-created dangerous directory. Skip the .APEX
      // segment when it's followed by 'worktrees'. Any nested .APEX directories
      ...
      if (dir === '.APEX') {
        ...
        if (
          ...
          break // Skip this .APEX, continue checking other segments
        }
      }
 ...
 * - 8.3 short names (e.g., GIT~1, APEX~1, SETTIN~1.JSON)
 * - Long path prefixes (e.g., \\?\C:\..., \\.\C:\..., //?/C:/..., //./C:/...)
 * - Trailing dots and spaces (e.g., .git., .APEX , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: GIT~1, APEX~1, SETTIN~1.JSON, BASHRC~1
  if (/~\d/.test(path)) {
    return true
  }
  ...
  // Examples: .git., .APEX , .bashrc..., settings.json.
 ...
 * - APEX config files (.APEX/settings.json, .APEX/commands/, .APEX/agents/)
 * - MCP CLI state files (managed internally by APEX Code)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `APEX requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for APEX config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isAPEXConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `APEX requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `APEX requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
export function pathInWorkingPath(path: string, workingPath: string): boolean {
  ...
  // checks on case-insensitive filesystems (macOS/Windows) like .APEX/CoMmAnDs
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .APEX/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `APEX requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `APEX requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `APEX requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `APEX requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .APEX is a dangerous directory
  ...
  // 1.6. Check for .APEX/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .APEX/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .APEX/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.APEX) rule in userSettings (e.g. from sandbox
  ...
  // "allow APEX to edit its own settings for this session" option actually works.
  const APEXFolderAllowRule = matchingRuleForInput(
    path,
    {
      ...toolPermissionContext,
      alwaysAllowRules: {
  ...
  if (APEXFolderAllowRule) {
    // Check if this rule is scoped under .APEX/ (project or global).
    // Accepts both the broad patterns ('/.APEX/**', '~/.APEX/**') and
    // narrowed ones like '/.APEX/skills/my-skill/**' so users can grant
    ...
    // '/.APEX/../**' from leaking this bypass outside .APEX/.
    const ruleContent = APEXFolderAllowRule.ruleValue.ruleContent
    if (
      ruleContent &&
      (ruleContent.startsWith(APEX_FOLDER_PERMISSION_PATTERN.slice(0, -2)) ||
        ruleContent.startsWith(
          GLOBAL_APEX_FOLDER_PERMISSION_PATTERN.slice(0, -2),
        )) &&
      ...
      return {
        ...
        decisionReason: {
          type: 'rule',
          rule: APEXFolderAllowRule,
        },
      }
    }
  }

  // 1.7. Check comprehensive safety validations (Windows patterns, APEX config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .APEX/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.APEX/settings.json, .git/, .vscode/, .idea/) falls
    ...
    const skillScope = getAPEXSkillScope(path)
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.APEX/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    const jobDir = process.env.APEX_JOB_DIR
    if (jobDir) {
      const jobsRoot = join(getAPEXConfigHomeDir(), 'jobs')
      ...
      // the case where ~/.APEX is a symlink (e.g. to /data/APEX-config).
  ...
  // ~/.APEX/, which is in DANGEROUS_DIRECTORIES. The APEX_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .APEX/launch.json — desktop preview config (dev server command + port).
  // The desktop's preview_start MCP tool instructs APEX to create/update
  // this file as part of the preview workflow. Without this carve-out the
  // .APEX/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .APEX/ only (not ~/.APEX/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.APEX', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Project temp directory (/tmp/APEX/{sanitized-cwd}/)
  ...
  // Tasks directory (~/.APEX/tasks/) for swarm task coordination
  const tasksDir = join(getAPEXConfigHomeDir(), 'tasks') + sep
  if (
  ...
  // Teams directory (~/.APEX/teams/) for swarm coordination
  const teamsReadDir = join(getAPEXConfigHomeDir(), 'teams') + sep
  if (
