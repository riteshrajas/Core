 ...
 * validation purposes, so commands like `echo foo > /tmp/APEX/x.txt` don't
 * prompt for permission when /tmp/APEX/ is already in the sandbox allowlist.
 *
 * Respects the deny-within-allow list: paths in denyWithinAllow (like
 * .APEX/settings.json) are still blocked even if their parent is in allowOnly.
 */
...
export function isPathAllowed(
  ...
  // Checks: Windows patterns, APEX config files, dangerous files (on original + symlink paths)
  if (operationType !== 'read') {
