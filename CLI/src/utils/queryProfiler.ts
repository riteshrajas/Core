 ...
 * pipeline from user input to first token arrival. Enable by setting APEX_CODE_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.APEX_CODE_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set APEX_CODE_PROFILE_QUERY=1)'
  }
