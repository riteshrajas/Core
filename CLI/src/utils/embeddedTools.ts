 ...
 *
 * When true:
 * - `find` and `grep` in APEX's Bash shell are shadowed by shell functions
 ...
 * - Prompt guidance steering APEX away from find/grep is omitted
 *
...
export function hasEmbeddedSearchTools(): boolean {
  ...
  const e = process.env.APEX_CODE_ENTRYPOINT
  return (
