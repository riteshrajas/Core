...
const resolveCanonicalRoot = memoizeWithLRU(
  (gitRoot: string): string => {
    try {
      ...
      // dialog and executing hooks from .APEX/settings.json on startup.
      //
      ...
      // Use the common dir itself as the stable identity (anthropics/APEX-code#27994).
...
export const stashToCleanState = async (message?: string): Promise<boolean> => {
  try {
    const stashMessage =
      message || `APEX Code auto-stash - ${new Date().toISOString()}`
