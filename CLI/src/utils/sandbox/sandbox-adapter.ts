/**
 * Adapter layer that wraps @anthropic-ai/sandbox-runtime with APEX CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and APEX CLI's
...
import { getAPEXTempDir } from '../permissions/filesystem.js'
...
/**
 * Resolve APEX Code-specific path patterns for sandbox-runtime.
 *
 * APEX Code uses special path prefixes in permission rules:
...
/**
 * Convert APEX Code settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and APEX temp directory as writable
  // The temp directory is needed for Shell.ts cwd tracking files
  const allowWrite: string[] = ['.', getAPEXTempDir()]
  const denyWrite: string[] = []
  const denyRead: string[] = []
  const allowRead: string[] = []

  // Always deny writes to settings.json files to prevent sandbox escape
  // This blocks settings in the original working directory (where APEX Code started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.APEX', 'settings.json'))
    denyWrite.push(resolve(cwd, '.APEX', 'settings.local.json'))
  }

  // Block writes to .APEX/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .APEX/commands and
  // .APEX/agents but not .APEX/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full APEX capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.APEX', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.APEX', 'skills'))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when APEX's unsandboxed git runs.
  //
...
// scrubbed if they appear after a sandboxed command. See anthropics/APEX-code#29316.
const bareGitRepoScrubPaths: string[] = []

/**
 * Delete bare-repo files planted at cwd during a sandboxed command, before
 * APEX's unsandboxed git calls can see them. See the SECURITY block above
 * bareGitRepoFiles. anthropics/APEX-code#29316.
 */
 ...
 * This is a APEX CLI-specific function that updates local settings.
 */
...
/**
 * APEX CLI sandbox manager - wraps sandbox-runtime with APEX-specific features
 */
