...
function createHttpsProxyAgent(
  ...
  if (isEnvTruthy(process.env.APEX_CODE_PROXY_RESOLVES_HOSTS)) {
 ...
 *   env var is set by `APEX ssh` on the remote CLI to route API calls through
...
export function getProxyFetchOptions(opts?: { forAnthropicAPI?: boolean }): {
  ...
  // ANTHROPIC_UNIX_SOCKET tunnels through the `APEX ssh` auth proxy, which
