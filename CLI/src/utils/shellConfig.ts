 ...
 * Used for managing APEX aliases and PATH entries
 */
...
import { getLocalAPEXPath } from './localInstaller.js'

export const APEX_ALIAS_REGEX = /^\s*alias\s+APEX\s*=/
...
/**
 * Filter out installer-created APEX aliases from an array of lines
 * Only removes aliases pointing to $HOME/.APEX/local/APEX
 ...
 */
export function filterAPEXAliases(lines: string[]): {
  filtered: string[]
  hadAlias: boolean
} {
  let hadAlias = false
  const filtered = lines.filter(line => {
    // Check if this is a APEX alias
    if (APEX_ALIAS_REGEX.test(line)) {
      // Extract the alias target - handle spaces, quotes, and various formats
      // First try with quotes
      let match = line.match(/alias\s+APEX\s*=\s*["']([^"']+)["']/)
      if (!match) {
        // Try without quotes (capturing until end of line or comment)
        match = line.match(/alias\s+APEX\s*=\s*([^#\n]+)/)
      }
...
/**
 * Check if a APEX alias exists in any shell config file
 * Returns the alias target if found, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findAPEXAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  ...
  for (const configPath of Object.values(configs)) {
    ...
    for (const line of lines) {
      if (APEX_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(/alias\s+APEX=["']?([^"'\s]+)/)
        if (match && match[1]) {
          return match[1]
        }
      }
...
/**
 * Check if a APEX alias exists and points to a valid executable
 * Returns the alias target if valid, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findValidAPEXAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  const aliasTarget = await findAPEXAlias(options)
