...
export async function runExtraUsage(): Promise<ExtraUsageResult> {
  ...
  const hasBillingAccess = hasAPEXAiBillingAccess()

  if (!hasBillingAccess && isTeamOrEnterprise) {
    // Mirror apps/APEX-ai useHasUnlimitedOverage(): if overage is enabled
  ...
  const url = isTeamOrEnterprise
    ? 'https://APEX.ai/admin-settings/usage'
    : 'https://APEX.ai/settings/usage'
