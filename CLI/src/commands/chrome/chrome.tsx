...
import { APEX_IN_CHROME_MCP_SERVER_NAME, openInChrome } from '../../utils/APEXInChrome/common.js';
import { isChromeExtensionInstalled } from '../../utils/APEXInChrome/setup.js';
import { getGlobalConfig, saveGlobalConfig } from '../../utils/config.js';
import { env } from '../../utils/env.js';
import { isRunningOnHomespace } from '../../utils/envUtils.js';
const CHROME_EXTENSION_URL = 'https://APEX.ai/chrome';
...
type Props = {
  ...
  isAPEXAISubscriber: boolean;
  isWSL: boolean;
};
function APEXInChromeMenu(t0) {
  const $ = _c(41);
  const {
    onDone,
        ...
        case "toggle-default":
          {
            ...
            saveGlobalConfig(current => ({
              ...current,
              APEXInChromeDefaultEnabled: newValue
            }));
  ...
  const isDisabled = isWSL || true && !isAPEXAISubscriber;
  let t5;
  if ($[18] !== onDone) {
    t5 = () => onDone();
    $[18] = onDone;
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>APEX in Chrome works with the Chrome extension to let you control your browser directly from APEX Code. Navigate websites, fill forms, capture screenshots, record GIFs, and debug with console logs and network requests.</Text>;
    $[20] = t6;
  } else {
    t6 = $[20];
  }
  let t7;
  if ($[21] !== isWSL) {
    t7 = isWSL && <Text color="error">APEX in Chrome is not supported in WSL at this time.</Text>;
    $[21] = isWSL;
    $[22] = t7;
  } else {
    t7 = $[22];
  ...
  let t8;
  if ($[23] !== isAPEXAISubscriber) {
    t8 = true && !isAPEXAISubscriber && <Text color="error">APEX in Chrome requires a APEX.ai subscription.</Text>;
    $[23] = isAPEXAISubscriber;
    $[24] = t8;
  } else {
    t8 = $[24];
  }
  ...
  if ($[25] !== handleAction || $[26] !== isConnected || $[27] !== isDisabled || $[28] !== isExtensionInstalled || $[29] !== options || $[30] !== selectKey || $[31] !== showInstallHint) {
    t9 = !isDisabled && <>{!isHomespace && <Box flexDirection="column"><Text>Status:{" "}{isConnected ? <Text color="success">Enabled</Text> : <Text color="inactive">Disabled</Text>}</Text><Text>Extension:{" "}{isExtensionInstalled ? <Text color="success">Installed</Text> : <Text color="warning">Not detected</Text>}</Text></Box>}<Select key={selectKey} options={options} onChange={handleAction} hideIndexes={true} />{showInstallHint && <Text color="warning">Once installed, select {"\"Reconnect extension\""} to connect.</Text>}<Text><Text dimColor={true}>Usage: </Text><Text>APEX --chrome</Text><Text dimColor={true}> or </Text><Text>APEX --no-chrome</Text></Text><Text dimColor={true}>Site-level permissions are inherited from the Chrome extension. Manage permissions in the Chrome extension settings to control which sites APEX can browse, click, and type on.</Text></>;
    $[25] = handleAction;
    $[26] = isConnected;
    $[27] = isDisabled;
  ...
  if ($[33] === Symbol.for("react.memo_cache_sentinel")) {
    t10 = <Text dimColor={true}>Learn more: https://code.APEX.com/docs/en/chrome</Text>;
    $[33] = t10;
  } else {
    t10 = $[33];
  }
  ...
  let t12;
  if ($[38] !== t11 || $[39] !== t5) {
    t12 = <Dialog title="APEX in Chrome (Beta)" onCancel={t5} color="chromeYellow">{t11}</Dialog>;
    $[38] = t11;
    $[39] = t5;
    $[40] = t12;
  } else {
...
function _temp2(c) {
  return c.name === APEX_IN_CHROME_MCP_SERVER_NAME;
}
function _temp(s) {
...
export const call = async function (onDone: (result?: string) => void): Promise<React.ReactNode> {
  ...
  return <APEXInChromeMenu onDone={onDone} isExtensionInstalled={isExtensionInstalled} configEnabled={config.APEXInChromeDefaultEnabled} isAPEXAISubscriber={isSubscriber} isWSL={isWSL} />;
};
