...
import { checkIsGitClean, checkNeedsAPEXAiLogin } from 'src/utils/background/remote/preconditions.js';
...
export function TeleportError(t0) {
  ...
  const handleLoginWithAPEXAI = t6;
  let t7;
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t7 = value => {
      if (value === "login") {
        handleLoginWithAPEXAI();
      } else {
        onCancel();
      }
    };
    ...
    case "needsLogin":
      {
        if (isLoggingIn) {
          ...
          if ($[14] !== handleLoginComplete) {
            t9 = <ConsoleOAuthFlow onDone={handleLoginComplete} mode="login" forceLoginMethod="APEXai" />;
        ...
        if ($[16] === Symbol.for("react.memo_cache_sentinel")) {
          t9 = <Box flexDirection="column"><Text dimColor={true}>Teleport requires a APEX.ai account.</Text><Text dimColor={true}>Your APEX Pro/Max subscription will be used by APEX Code.</Text></Box>;
          $[16] = t9;
        } else {
          t9 = $[16];
        }
        ...
        if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
          t10 = <Dialog title="Log in to APEX" onCancel={onCancel}>{t9}<Select options={[{
              label: "Login with APEX account",
              value: "login"
            }, {
              label: "Exit",
              value: "exit"
...
export async function getTeleportErrors(): Promise<Set<TeleportLocalErrorType>> {
  ...
  const [needsLogin, isGitClean] = await Promise.all([checkNeedsAPEXAiLogin(), checkIsGitClean()]);
  if (needsLogin) {
