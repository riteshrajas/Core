...
const THINKING_WALK_FRAMES = ['o/ ', 'o> ', 'o\\ ', 'o< '];
const ERROR_RED = {
  r: 171,
  g: 43,
  b: 63
...
export function SpinnerGlyph({
  ...
  const spinnerChar = isThinking ? THINKING_WALK_FRAMES[frame % THINKING_WALK_FRAMES.length] : SPINNER_FRAMES[frame % SPINNER_FRAMES.length];
  const glyphWidth = isThinking ? 4 : 2;

  if (reducedMotion) {
    const isDim = Math.floor(time / (REDUCED_MOTION_CYCLE_MS / 2)) % 2 === 1;
    const reducedMotionChar = isThinking ? 'o> ' : REDUCED_MOTION_DOT;
    return <Box flexWrap="wrap" height={1} width={glyphWidth}><Text color={messageColor} dimColor={isDim}>{reducedMotionChar}</Text></Box>;
  }
  ...
  if (stalledIntensity > 0) {
    ...
    if (baseRGB) {
      ...
      return <Box flexWrap="wrap" height={1} width={glyphWidth}><Text color={toRGBColor(interpolated)}>{spinnerChar}</Text></Box>;
    }
    const color = stalledIntensity > 0.5 ? 'error' : messageColor;
    return <Box flexWrap="wrap" height={1} width={glyphWidth}><Text color={color}>{spinnerChar}</Text></Box>;
  }

  return <Box flexWrap="wrap" height={1} width={glyphWidth}><Text color={messageColor}>{spinnerChar}</Text></Box>;
}
