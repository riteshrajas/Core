...
import { useAPEXAiLimits } from '../../services/APEXAiLimitsHook.js';
...
export function Notifications(t0) {
  ...
  const APEXAiLimits = useAPEXAiLimits();
  let t5;
  let t6;
  ...
  const isInOverageMode = APEXAiLimits.isUsingOverage;
  let t7;
  ...
  return <>
      ...
      {(apiKeyStatus === 'invalid' || apiKeyStatus === 'missing') && <Box>
          <Text color="error" wrap="truncate">
            {isEnvTruthy(process.env.APEX_CODE_REMOTE) ? 'Authentication error · Try again' : 'Not logged in · Run /login'}
          </Text>
        </Box>}
      {debug && <Box>
          <Text color="warning" wrap="truncate">
