...
import { getMemoryFiles, type MemoryFileInfo } from '../../utils/APEXmd.js';
...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getAPEXConfigHomeDir(), "APEX.md");
  const projectMemoryPath = join(getOriginalCwd(), "APEX.md");
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.APEX/APEX.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./APEX.md`;
      } else {
        if (file.parent) {
