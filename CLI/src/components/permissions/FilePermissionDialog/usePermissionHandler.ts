...
export type PermissionHandlerOptions = {
  ...
  scope?: 'APEX-folder' | 'global-APEX-folder'
}
...
function handleAcceptSession(
  ...
  // For APEX-folder scope, grant session-level access to all .APEX/ files
  if (
    options?.scope === 'APEX-folder' ||
    options?.scope === 'global-APEX-folder'
  ) {
    const pattern =
      options.scope === 'global-APEX-folder'
        ? GLOBAL_APEX_FOLDER_PERMISSION_PATTERN
