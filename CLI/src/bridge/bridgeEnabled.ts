 ...
 *
 * Remote Control requires a APEX.ai subscription (the bridge auths to CCR
 * with the APEX.ai OAuth token). isAPEXAISubscriber() excludes
 ...
 * (setup-token, APEX_CODE_OAUTH_TOKEN env var, or pre-scope-expansion
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isAPEXAISubscriber()) {
      return 'Remote Control requires a APEX.ai subscription. Run `APEX auth login` to sign in with your APEX.ai account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `APEX setup-token` or APEX_CODE_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `APEX auth login` to use Remote Control.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine your organization for Remote Control eligibility. Run `APEX auth login` to refresh your account information.'
    }
...
// program, before enableConfigs() runs. isAPEXAISubscriber() → getGlobalConfig()
...
function isAPEXAISubscriber(): boolean {
  try {
    return authModule.isAPEXAISubscriber()
  } catch {
    return false
  }
}
 ...
 * APEX.ai frontend routes on `session_*`, while v2 worker endpoints hand out
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of APEX Code (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`APEX update\` to update.`
    }
  }
  return null
}
...
export function isCcrMirrorEnabled(): boolean {
  return feature('CCR_MIRROR')
    ? isEnvTruthy(process.env.APEX_CODE_CCR_MIRROR) ||
