        ...
        (token: string) =>
          axios.post<{
            ...
            `${deps.baseUrl}/v1/environments/bridge`,
            {
              ...
              // Advertise session capacity so APEX.ai/code can show
              ...
              // worker_type lets APEX.ai filter environments by origin
    ...
    case 403:
      throw new BridgeFatalError(
        isExpiredErrorType(errorType)
          ? 'Remote Control session has expired. Please restart with `APEX remote-control` or /remote-control.'
