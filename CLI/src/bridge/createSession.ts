 ...
 *
 * Used by both `APEX remote-control` (empty session so the user has somewhere to
...
export async function createBridgeSession({
  ...
  const { getAPEXAIOAuthTokens } = await import('../utils/auth.js')
  ...
  const accessToken =
    getAccessToken?.() ?? getAPEXAIOAuthTokens()?.accessToken
  if (!accessToken) {
...
export async function getBridgeSession(
  ...
  const accessToken =
    opts?.getAccessToken?.() ?? getAPEXAIOAuthTokens()?.accessToken
  if (!accessToken) {
 ...
 * explicit client action. Both `APEX remote-control` (standalone bridge) and the
 ...
 * connection is active, so the title stays in sync on APEX.ai/code.
 *
