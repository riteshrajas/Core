...
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control is only available with APEX.ai subscriptions. Please use `/login` to sign in with your APEX.ai account.'

/** Full error printed when `APEX remote-control` is run without auth. */
...
export type WorkSecret = {
  ...
  APEX_code_args?: Record<string, string> | null
...
/**
 * How `APEX remote-control` chooses session working directories.
 ...
 * `metadata.worker_type` at environment registration so APEX.ai can filter
 ...
 */
export type BridgeWorkerType = 'APEX_code' | 'APEX_code_assistant'
...
export type BridgeConfig = {
   ...
   * instead of creating a new one. Used by `APEX remote-control
