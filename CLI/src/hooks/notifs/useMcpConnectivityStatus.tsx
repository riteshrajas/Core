...
import { hasAPEXAiMcpEverConnected } from '../../services/mcp/APEXai.js';
...
export function useMcpConnectivityStatus(t0) {
  ...
  if ($[0] !== addNotification || $[1] !== mcpClients) {
    t2 = () => {
      ...
      const failedAPEXAiClients = mcpClients.filter(_temp2);
      const needsAuthLocalServers = mcpClients.filter(_temp3);
      const needsAuthAPEXAiServers = mcpClients.filter(_temp4);
      if (failedLocalClients.length === 0 && failedAPEXAiClients.length === 0 && needsAuthLocalServers.length === 0 && needsAuthAPEXAiServers.length === 0) {
        return;
      }
      ...
      if (failedAPEXAiClients.length > 0) {
        addNotification({
          key: "mcp-APEXai-failed",
          jsx: <><Text color="error">{failedAPEXAiClients.length} APEX.ai{" "}{failedAPEXAiClients.length === 1 ? "connector" : "connectors"}{" "}unavailable</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
      ...
      if (needsAuthAPEXAiServers.length > 0) {
        addNotification({
          key: "mcp-APEXai-needs-auth",
          jsx: <><Text color="warning">{needsAuthAPEXAiServers.length} APEX.ai{" "}{needsAuthAPEXAiServers.length === 1 ? "connector needs" : "connectors need"}{" "}auth</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
    };
...
function _temp4(client_2) {
  return client_2.type === "needs-auth" && client_2.config.type === "APEXai-proxy" && hasAPEXAiMcpEverConnected(client_2.name);
}
function _temp3(client_1) {
  return client_1.type === "needs-auth" && client_1.config.type !== "APEXai-proxy";
}
function _temp2(client_0) {
  return client_0.type === "failed" && client_0.config.type === "APEXai-proxy" && hasAPEXAiMcpEverConnected(client_0.name);
}
function _temp(client) {
  return client.type === "failed" && client.config.type !== "sse-ide" && client.config.type !== "ws-ide" && client.config.type !== "APEXai-proxy";
}
