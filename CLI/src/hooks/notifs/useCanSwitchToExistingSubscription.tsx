...
async function _temp2() {
  ...
  const subscriptionType = await getExistingAPEXSubscription();
  ...
  return {
    ...
    jsx: <Text color="suggestion">Use your existing APEX {subscriptionType} plan with APEX Code<Text color="text" dimColor={true}>{" "}· /login to activate</Text></Text>,
    priority: "low"
  };
}
...
async function getExistingAPEXSubscription(): Promise<'Max' | 'Pro' | null> {
  ...
  if (profile.account.has_APEX_max) {
    return 'Max';
  }
  if (profile.account.has_APEX_pro) {
    return 'Pro';
  }
  return null;
}
