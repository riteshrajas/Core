...
import { getRateLimitWarning, getUsingOverageText } from 'src/services/APEXAiLimits.js';
import { useAPEXAiLimits } from 'src/services/APEXAiLimitsHook.js';
import { getSubscriptionType } from 'src/utils/auth.js';
import { hasAPEXAiBillingAccess } from 'src/utils/billing.js';
...
export function useRateLimitWarningNotification(model) {
  ...
  let t0;
  if ($[0] !== APEXAiLimits || $[1] !== model) {
    t0 = getRateLimitWarning(APEXAiLimits, model);
    $[0] = APEXAiLimits;
    $[1] = model;
    $[2] = t0;
  } else {
  ...
  let t1;
  if ($[3] !== APEXAiLimits) {
    t1 = getUsingOverageText(APEXAiLimits);
    $[3] = APEXAiLimits;
    $[4] = t1;
  } else {
    t1 = $[4];
  ...
  let t4;
  let t5;
  if ($[7] !== addNotification || $[8] !== APEXAiLimits.isUsingOverage || $[9] !== hasShownOverageNotification || $[10] !== usingOverageText) {
    t4 = () => {
      if (getIsRemoteMode()) {
        return;
      }
      if (APEXAiLimits.isUsingOverage && !hasShownOverageNotification && (!isTeamOrEnterprise || hasBillingAccess)) {
        addNotification({
          key: "limit-reached",
        ...
        if (!APEXAiLimits.isUsingOverage && hasShownOverageNotification) {
          setHasShownOverageNotification(false);
        }
      }
    };
    t5 = [APEXAiLimits.isUsingOverage, usingOverageText, hasShownOverageNotification, addNotification, hasBillingAccess, isTeamOrEnterprise];
    $[7] = addNotification;
    $[8] = APEXAiLimits.isUsingOverage;
