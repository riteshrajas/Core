...
/**
 * Surfaces plugin-install prompts driven by `<APEX-code-hint />` tags
 * that CLIs/SDKs emit to stderr. See docs/APEX-code-hints.md.
 *
...
import { clearPendingHint, getPendingHintSnapshot, markShownThisSession, subscribeToPendingHint } from '../utils/APEXCodeHints.js';
  ...
  if ($[0] !== pendingHint || $[1] !== tryResolve) {
    t0 = () => {
      ...
      tryResolve(async () => {
        ...
        if (resolved) {
          logForDebugging(`[useAPEXCodeHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`);
