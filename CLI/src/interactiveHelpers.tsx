...
import { getExternalAPEXMdIncludes, getMemoryFiles, shouldShowAPEXMdExternalIncludesWarning } from './utils/APEXmd.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], APEXInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  // and checks APEX.md external includes. bypassPermissions mode
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    // Check for APEX.md includes that need approval
    if (await shouldShowAPEXMdExternalIncludesWarning()) {
      const externalIncludes = getExternalAPEXMdIncludes(await getMemoryFiles(true));
      const {
        APEXMdExternalIncludesDialog
      } = await import('./components/APEXMdExternalIncludesDialog.js');
      await showSetupDialog(root, done => <APEXMdExternalIncludesDialog onDone={done} isStandaloneDialog externalIncludes={externalIncludes} />);
    }
  }
  ...
  // processes but ignored by APEX Code itself (see auth.ts).
  ...
  if (feature('KAIROS') || feature('KAIROS_CHANNELS')) {
    ...
    if (devChannels && devChannels.length > 0) {
      ...
      if (!isChannelsEnabled() || !getAPEXAIOAuthTokens()?.accessToken) {
  ...
  // Show Chrome onboarding for first-time APEX in Chrome users
  if (APEXInChrome && !getGlobalConfig().hasCompletedAPEXInChromeOnboarding) {
    const {
      APEXInChromeOnboarding
    } = await import('./components/APEXInChromeOnboarding.js');
    await showSetupDialog(root, done => <APEXInChromeOnboarding onDone={done} />);
  }
...
export function getRenderContext(exitOnCtrlC: boolean): {
  ...
  const frameTimingLogPath = process.env.APEX_CODE_FRAME_TIMING_LOG;
  return {
