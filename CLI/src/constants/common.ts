...
export function getLocalISODate(): string {
  ...
  if (process.env.APEX_CODE_OVERRIDE_DATE) {
    return process.env.APEX_CODE_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.APEX_CODE_OVERRIDE_DATE
    ? new Date(process.env.APEX_CODE_OVERRIDE_DATE)
    : new Date()
