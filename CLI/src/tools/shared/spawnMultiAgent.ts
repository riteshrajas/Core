 ...
 * When outside tmux: Creates a APEX-swarm session with all teammates in a tiled layout.
 */
async function handleSpawnSplitPane(
  ...
  // - Outside both: creates APEX-swarm session with tiled teammates
  ...
  // Build the command to spawn APEX Code with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces APEX_CODE_* env vars)
  const teammateArgs = [
  ...
  // Includes APEXCODE, APEX_CODE_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new APEX Code instance.
