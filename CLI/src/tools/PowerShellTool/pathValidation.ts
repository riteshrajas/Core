...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .APEX is a dangerous directory
  // and internal editable paths live under ~/.APEX/ — matching the ordering in
  ...
  // have explicitly configured writable directories (e.g. /tmp/APEX/) —
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.APEX; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.APEX/settings.json (APEX's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.APEX`, validate subsequent statements with
  // cwd='./.APEX'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, APEX Code may only access files in the allowed working directories for this session: ${dirListStr}.`
