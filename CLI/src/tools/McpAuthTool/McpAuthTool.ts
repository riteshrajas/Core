...
export function createMcpAuthTool(
  ...
  return {
    ...
    async call(_input, context) {
      // APEX.ai connectors use a separate auth flow (handleAPEXAIAuth in
      ...
      if (config.type === 'APEXai-proxy') {
        return {
          data: {
            ...
            message: `This is a APEX.ai MCP connector. Ask the user to run /mcp and select "${serverName}" to authenticate.`,
          },
        }
      }
