enum ApexCommand { openApp, readFile, runShell, checkProgress, unknown }
  ...
  Map<String, dynamic> parse(String text) {
    ...
    if (input.contains('open') || input.contains('launch')) {
      return {
        ...
        'params': {
          'app': _extractTarget(input, ['open', 'launch']),
        },
      };
    }

    if (input.contains('read') ||
        input.contains('pull up') ||
        input.contains('show file')) {
      return {
        'command': ApexCommand.readFile,
        'params': {
          'file': _extractTarget(input, ['read', 'pull up', 'show file']),
        },
      };
    }
    ...
    if (input.contains('run') || input.contains('execute')) {
      return {
        ...
        'params': {
          'command': _extractTarget(input, ['run', 'execute']),
        },
      };
    }

    if (input.contains('progress') || input.contains('status')) {
      return {'command': ApexCommand.checkProgress, 'params': {}};
    }
